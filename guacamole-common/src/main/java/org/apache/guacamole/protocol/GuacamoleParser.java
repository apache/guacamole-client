...
/**
 * Parser for the Guacamole protocol. Arbitrary instruction data is appended, and instructions are
 * returned as a result. Invalid instructions result in exceptions.
 */
public class GuacamoleParser implements Iterator<GuacamoleInstruction> {
   ...
   * @return The number of characters appended, or 0 if complete instructions have already been
   * parsed and must be read via next() before more data can be appended.
  ...
  private enum State {
    ...
    /**
     * The parser is currently waiting for data to complete the length prefix of the current element
     * of the instruction.
     */
    PARSING_LENGTH,

    /**
     * The parser has finished reading the length prefix and is currently waiting for data to
     * complete the content of the instruction.
     */
    PARSING_CONTENT,
