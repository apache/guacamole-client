...
/**
 * GuacamoleSocket which intercepts errors received early in the Guacamole session. Upstream errors
 * which are intercepted early enough result in exceptions thrown immediately within the
 * FailoverGuacamoleSocket's constructor, allowing a different socket to be substituted prior to
...
public class FailoverGuacamoleSocket extends DelegatingGuacamoleSocket {
  ...
  /**
   * The default maximum number of characters of Guacamole instruction data to store if no explicit
   * limit is provided.
   */
  private static final int DEFAULT_INSTRUCTION_QUEUE_LIMIT = 131072;

  /**
   * Queue of all instructions read while this FailoverGuacamoleSocket was being constructed.
   */
  private final Queue<GuacamoleInstruction> instructionQueue =
      new LinkedList<GuacamoleInstruction>();
  /**
   * GuacamoleReader which reads instructions from the queue populated when the
   * FailoverGuacamoleSocket was constructed. Once the queue has been emptied, reads are delegated
   * directly to the reader of the wrapped socket.
   */
  ...
  /**
   * Creates a new FailoverGuacamoleSocket which reads Guacamole instructions from the given socket,
   * searching for errors from the upstream remote desktop until the given instruction queue limit
   * is reached. If an upstream error is encountered, it is thrown as a GuacamoleUpstreamException.
   * This constructor will block until an error is encountered, until insufficient space remains in
   * the instruction queue, or until the connection appears to have been successful. Once the
   * FailoverGuacamoleSocket has been created, all reads, writes, etc. will be delegated to the
   * provided socket.
   *
   * @param socket                The GuacamoleSocket of the Guacamole connection this
   *                              FailoverGuacamoleSocket should handle.
   * @param instructionQueueLimit The maximum number of characters of Guacamole instruction data to
   *                              store within the instruction queue while searching for errors.
   *                              Once this limit is exceeded, the connection is assumed to be
   *                              successful.
   * @throws GuacamoleException         If an error occurs while reading data from the provided
   *                                    socket.
   * @throws GuacamoleUpstreamException If the connection to guacd succeeded, but an error occurred
  ...
  /**
   * Creates a new FailoverGuacamoleSocket which reads Guacamole instructions from the given socket,
   * searching for errors from the upstream remote desktop until a maximum of 128KB of instruction
   * data has been queued. If an upstream error is encountered, it is thrown as a
   * GuacamoleUpstreamException. This constructor will block until an error is encountered, until
   * insufficient space remains in the instruction queue, or until the connection appears to have
   * been successful. Once the FailoverGuacamoleSocket has been created, all reads, writes, etc.
   * will be delegated to the provided socket.
   *
   * @param socket The GuacamoleSocket of the Guacamole connection this FailoverGuacamoleSocket
   *               should handle.
   * @throws GuacamoleException         If an error occurs while reading data from the provided
   *                                    socket.
   * @throws GuacamoleUpstreamException If the connection to guacd succeeded, but an error occurred
  ...
  /**
   * Parses the given "error" instruction, throwing an exception if the instruction represents an
   * error from the upstream remote desktop.
   *
   * @param instruction The "error" instruction to parse.
   * @throws GuacamoleUpstreamException If the "error" instruction represents an error from the
   *                                    upstream remote desktop.
   */
