...
/**
 * A GuacamoleSocket which pre-configures the connection based on a given GuacamoleConfiguration,
 * completing the initial protocol handshake before accepting data for read or write.
 * <p>
 * This is useful for forcing a connection to the Guacamole proxy server with a specific
 * configuration while disallowing the client that will be using this GuacamoleSocket from manually
 * controlling the initial protocol handshake.
 */
public class ConfiguredGuacamoleSocket extends DelegatingGuacamoleSocket {
  ...
  /**
   * The configuration to use when performing the Guacamole protocol handshake.
   */
  private GuacamoleConfiguration config;

  /**
   * The unique identifier associated with this connection, as determined by the "ready" instruction
   * received from the Guacamole proxy.
   */
  private String id;

  /**
   * The protocol version that will be used to communicate with guacd.  The default is 1.0.0, and,
   * if the server does not provide a specific version it will be assumed that it operates at this
   * version and certain features may be unavailable.
   */
  private GuacamoleProtocolVersion protocolVersion =
      GuacamoleProtocolVersion.VERSION_1_0_0;

  /**
   * Creates a new ConfiguredGuacamoleSocket which uses the given GuacamoleConfiguration to complete
   * the initial protocol handshake over the given GuacamoleSocket. A default
   * GuacamoleClientInformation object is used to provide basic client information.
   *
   * @param socket The GuacamoleSocket to wrap.
   * @param config The GuacamoleConfiguration to use to complete the initial protocol handshake.
   * @throws GuacamoleException If an error occurs while completing the initial protocol handshake.
   */
  ...
  /**
   * Creates a new ConfiguredGuacamoleSocket which uses the given GuacamoleConfiguration and
   * GuacamoleClientInformation to complete the initial protocol handshake over the given
   * GuacamoleSocket.
   *
   * @param socket The GuacamoleSocket to wrap.
   * @param config The GuacamoleConfiguration to use to complete the initial protocol handshake.
   * @param info   The GuacamoleClientInformation to use to complete the initial protocol
   *               handshake.
   * @throws GuacamoleException If an error occurs while completing the initial protocol handshake.
   */
  public ConfiguredGuacamoleSocket(GuacamoleSocket socket,
  ...
  /**
   * Parses the given "error" instruction, throwing a GuacamoleException that corresponds to its
   * status code and message.
   *
   * @param instruction The "error" instruction to parse.
   * @throws GuacamoleException A GuacamoleException that corresponds to the status code and message
    ...
    if (args.size() >= 2) {
      try {
        ...
        if (parsedStatus != null) {
          ...
          logger.debug("Received \"error\" instruction with unknown/invalid status code: {}",
              statusCode);
        }
  ...
  /**
   * Waits for the instruction having the given opcode, returning that instruction once it has been
   * read. If the instruction is never read, an exception is thrown.
   * <p>
   * Respects server control instructions that are allowed during the handshake phase, namely
   * {@code error} and {@code disconnect}.
   *
   ...
   * @throws GuacamoleException If an error occurs while reading, or if the expected instruction is
   *                            not read.
   */
  private GuacamoleInstruction expect(GuacamoleReader reader, String opcode)
    ...
    if ("disconnect".equals(instruction.getOpcode())) {
      throw new GuacamoleConnectionClosedException(
          "Server disconnected while waiting for \"" + opcode + "\".");
    }
    ...
    if (!instruction.getOpcode().equals(opcode)) {
      throw new GuacamoleServerException(
          "Expected \"" + opcode + "\" instruction but instead received \""
   ...
   *
   * @return The GuacamoleConfiguration used to configure this ConfiguredGuacamoleSocket.
   */
  ...
  /**
   * Returns the unique ID associated with the Guacamole connection negotiated by this
   * ConfiguredGuacamoleSocket. The ID is provided by the "ready" instruction returned by the
   * Guacamole proxy.
   *
  ...
  /**
   * Returns the version of the Guacamole protocol associated with the Guacamole connection
   * negotiated by this ConfiguredGuacamoleSocket. This version is the lowest version common to both
   * ConfiguredGuacamoleSocket and the relevant Guacamole proxy instance (guacd).
   *
   * @return The protocol version that this ConfiguredGuacamoleSocket will use to communicate with
   * guacd.
   */
