...
/**
 * GuacamoleWriter which applies a given GuacamoleFilter to observe or alter all written
 * instructions. Instructions may also be dropped or denied by the filter.
 */
public class FilteredGuacamoleWriter implements GuacamoleWriter {
  ...
  /**
   * Parser for reading instructions prior to writing, such that they can be passed on to the
   * filter.
   */
  private final GuacamoleParser parser = new GuacamoleParser();

  /**
   * Wraps the given GuacamoleWriter, applying the given filter to all written instructions. Future
   * writes will only write instructions which pass the filter.
   *
   * @param writer The GuacamoleWriter to wrap.
   * @param filter The filter which dictates which instructions are written, and how.
   */
