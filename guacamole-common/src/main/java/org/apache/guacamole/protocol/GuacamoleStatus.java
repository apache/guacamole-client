...
/**
 * All possible statuses returned by various Guacamole instructions, each having a corresponding
 * code.
 */
public enum GuacamoleStatus {
  ...
  /**
   * The operation was unsuccessful due to an error or otherwise unexpected condition of the
   * upstream server.
   */
  ...
  /**
   * The operation could not be performed because the upstream server is not available to service
   * the request.
   */
  ...
  /**
   * Permission was denied to perform the operation, as the user is not yet authorized (not yet
   * logged in, for example). As HTTP 401 has implications for HTTP-specific authorization schemes,
   * this status continues to map to HTTP 403 ("Forbidden"). To do otherwise would risk unintended
   * effects.
   */
  ...
  /**
   * Permission was denied to perform the operation, and this operation will not be granted even if
  ...
  /**
   * Initializes a GuacamoleStatusCode with the given HTTP and Guacamole status/error code values.
   *
  ...
  /**
   * Returns the GuacamoleStatus corresponding to the given Guacamole protocol status code. If no
   * such GuacamoleStatus is defined, null is returned.
   *
   * @param code The Guacamole protocol status code to translate into a GuacamoleStatus.
   * @return The GuacamoleStatus corresponding to the given Guacamole protocol status code, or null
   * if no such GuacamoleStatus is defined.
   */
  ...
  /**
   * Returns an instance of the {@link GuacamoleException} subclass corresponding to this Guacamole
   * protocol status code. All status codes have a corresponding GuacamoleException except for
   * {@link SUCCESS}. The returned GuacamoleException will have the provided human-readable
   * message.
   *
   * @param message A human readable description of the error that occurred.
   * @return An instance of the {@link GuacamoleException} subclass that corresponds to this status
   * code and has the provided human-readable message.
   * @throws IllegalStateException If invoked on {@link SUCCESS}, which has no corresponding
   *                               GuacamoleException.
   */
