...
/**
 * A WebSocket implementation of GuacamoleTunnel functionality, compatible with the
 * Guacamole.WebSocketTunnel object included with the JavaScript API. Messages sent/received are
 * simply chunks of the Guacamole protocol instruction stream.
 */
public abstract class GuacamoleWebSocketTunnelEndpoint extends Endpoint {
  ...
  /**
   * The opcode of the instruction used to indicate a connection stability test ping request or
   * response. Note that this instruction is encapsulated within an internal tunnel instruction
   * (with the opcode being the empty string), thus this will actually be the value of the first
   * element of the received instruction.
   */
  ...
  /**
   * The underlying GuacamoleTunnel. WebSocket reads/writes will be handled as reads/writes to this
   * tunnel. This value may be null if no connection has been established.
   */
  private GuacamoleTunnel tunnel;

  /**
   * Remote (client) side of this connection. This value will always be non-null if tunnel is
   * non-null.
   */
   ...
   * @param guacamoleStatusCode The numeric Guacamole status to send.
  ...
  /**
   * Sends the given Guacaomle Status and closes the given connection.
   *
   * @param session    The outbound WebSocket connection to close.
   * @param guacStatus The status to use for the connection.
   */
  ...
  /**
   * Sends a Guacamole instruction along the outbound WebSocket connection to the connected
   * Guacamole client. If an instruction is already in the process of being sent by another thread,
   * this function will block until in-progress instructions are complete.
   *
   * @param instruction The instruction to send.
   * @throws IOException If an I/O error occurs preventing the given instruction from being sent.
   */
  ...
  /**
   * Returns a new tunnel for the given session. How this tunnel is created or retrieved is
   * implementation-dependent.
   *
   * @param session The session associated with the active WebSocket connection.
   * @param config  Configuration information associated with the instance of the endpoint created
   *                for handling this single connection.
   * @return A connected tunnel, or null if no such tunnel exists.
   * @throws GuacamoleException If an error occurs while retrieving the tunnel, or if access to the
   *                            tunnel is denied.
   */
  ...
  public void onMessage(String message) {
    ...
    GuacamoleWriter writer = new FilteredGuacamoleWriter(tunnel.acquireWriter(),
        new GuacamoleFilter() {
