...
public class EnumGuacamolePropertyTest {
   ...
   */
  private static final EnumGuacamoleProperty<Fish> FAVORITE_FISH = new EnumGuacamoleProperty<Fish>(
      Fish.class) {
  ...
  /**
   * Verifies that EnumGuacamoleProperty correctly parses string values that are associated with
   * their corresponding enum constants using the {@link PropertyValue} annotation.
   *
   * @throws GuacamoleException If a valid test value is incorrectly recognized by parseValue() as
   *                            invalid.
   */
  @Test
  ...
  /**
   * Verifies that the absence of a property value (null) is parsed by EnumGuacamoleProperty as the
   * absence of an enum constant (also null).
   *
   * @throws GuacamoleException If a valid test value is incorrectly recognized by parseValue() as
   *                            invalid.
   */
  ...
  /**
   * Verifies that GuacamoleException is thrown when attempting to parse an invalid value, and that
   * the error message contains a sorted list of all allowed values.
   */
  @Test
  ...
  /**
   * Verifies that EnumGuacamoleProperty can be constructed for enums that are not annotated with
   * {@link PropertyValue}.
   *
   * @throws GuacamoleException If a valid test value is incorrectly recognized by parseValue() as
   *                            invalid.
  ...
  /**
   * Verifies that an IllegalArgumentException is thrown if key/value pairs are provided in the
   * wrong order (value followed by key instead of key followed by value).
   */
  @Test
  ...
  /**
   * Verifies that an IllegalArgumentException is thrown if constants from the wrong enum are
   * provided in an explicit mapping.
   */
  @Test
  ...
  /**
   * Verifies that an IllegalArgumentException is thrown if non-String keys are provided in an
   * explicit mapping.
   */
  @Test
  ...
  /**
   * Verifies that an IllegalArgumentException is thrown if the length of the {@code additional}
   * array is not even.
   */
  @Test
  public void testUnannotatedEnumBadLength() {
    ...
    try {
      ...
      fail(
          "EnumGuacamoleProperty should not accept additional key/value pairs from an array that is not even in length.");
  ...
  /**
   * Verifies that explicit string/constant mappings take priority over the {@link PropertyValue}
   * annotation when both are used.
   *
   * @throws GuacamoleException If a valid test value is incorrectly recognized by parseValue() as
   *                            invalid.
   */
  ...
  /**
   * Example enum consisting of a small set of possible fish. All values of this enum are annotated
   * with {@link PropertyValue}.
   */
  public static enum Fish {

    /**
     * Salmon are large, anadromous fish prized for their pink/red/orange flesh.
     *
    ...
    /**
     * Trout are freshwater fish related to salmon, popular both as food and as game fish.
     *
    ...
    /**
     * Mackerel are pelagic fish, typically having vertical stripes along their backs.
     *
    ...
    /**
     * Tuna are large, predatory, saltwater fish in the same family as mackerel. They are one of the
     * few fish that can maintain a body temperature higher than the surrounding water.
     *
    ...
    /**
     * Sardines are small, herring-like fish commonly served in cans. Sardines are considered prey
     * fish and feed almost exclusively on zooplankton.
     *
  ...
  /**
   * Example enum consisting of a small set of possible vegetables. None of the values of this enum
   * are annotated with {@link PropertyValue}.
   */
  public static enum Vegetable {

    /**
     * Potatoes are starchy root vegetables native to the Americas. The tuber itself is edible, but
     * other parts can be toxic.
     *
    ...
    /**
     * Carrots are root vegetables, tapered in shape and generally orange in color.
     *
