...
/**
 * An extremely simple UserContext implementation which provides access to a defined and restricted
 * set of GuacamoleConfigurations. Access to querying or modifying either users or permissions is
 * denied.
 */
public class SimpleUserContext extends AbstractUserContext {
  ...
  /**
   * The unique identifier (username) of the user whose permissions dictate the configurations
   * accessible within this UserContext.
   */
  private final String username;

  /**
   * The Directory with access to all connections within the root group associated with this
   * UserContext.
   */
  private final Directory<Connection> connectionDirectory;

  /**
   * Creates a new SimpleUserContext which provides access to only those configurations within the
   * given Map. The username is set to the ANONYMOUS_IDENTIFIER defined by AuthenticatedUser,
   * effectively declaring the current user as anonymous. Parameter tokens within the given
   * GuacamoleConfigurations will not be interpreted.
   *
   * @param authProvider The AuthenticationProvider creating this UserContext.
   * @param configs      A Map of all configurations for which the user associated with this
  ...
  /**
   * Creates a new SimpleUserContext for the user with the given username which provides access to
   * only those configurations within the given Map. Parameter tokens within the given
   * GuacamoleConfigurations will not be interpreted.
   *
   * @param authProvider The AuthenticationProvider creating this UserContext.
   * @param username     The username of the user associated with this UserContext.
   * @param configs      A Map of all configurations for which the user associated with this
   *                     UserContext has read access.
   */
  public SimpleUserContext(AuthenticationProvider authProvider,
   ...
   * Creates a new SimpleUserContext for the user with the given username which provides access to
   * only those configurations within the given Map. Parameter tokens within the given
   * GuacamoleConfigurations will be interpreted if explicitly requested.
   *
   * @param authProvider    The AuthenticationProvider creating this UserContext.
   * @param username        The username of the user associated with this UserContext.
   * @param configs         A Map of all configurations for which the user associated with this
   *                        UserContext has read access.
   * @param interpretTokens Whether parameter tokens in the underlying GuacamoleConfigurations
