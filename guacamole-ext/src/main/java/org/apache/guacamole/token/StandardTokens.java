...
/**
 * Utility class which provides access to standardized token names, as well as facilities for
 * generating those tokens from common objects.
 *
 * @deprecated Standard tokens are now supplied by default to the connect() functions of connections
 * and connection groups. Manually generating the standard tokens is not necessary.
 */
@Deprecated
public class StandardTokens {
  ...
  /**
   * The date format that should be used for the date token. This format must be compatible with
   * Java's SimpleDateFormat.
   */
  private static final String DATE_FORMAT = "yyyyMMdd";

  /**
   * The date format that should be used for the time token. This format must be compatible with
   * Java's SimpleDateFormat.
   */
  ...
  /**
   * Adds tokens which are standardized by guacamole-ext to the given TokenFilter and which do not
   * require a corresponding Credentials object. These the server date and time (GUAC_DATE and
   * GUAC_TIME respectively).
   *
   * @param filter The TokenFilter to add standard tokens to.
   */
  ...
  /**
   * Adds tokens which are standardized by guacamole-ext to the given TokenFilter using the values
   * from the given Credentials object. These standardized tokens include the current username
   * (GUAC_USERNAME), password (GUAC_PASSWORD), and the server date and time (GUAC_DATE and
   * GUAC_TIME respectively). If either the username or password are not set within the given
   * credentials, the corresponding token(s) will remain unset.
   *
   * @param filter      The TokenFilter to add standard tokens to.
   * @param credentials The Credentials to use when populating the GUAC_USERNAME and GUAC_PASSWORD
   *                    tokens.
   */
  ...
  /**
   * Adds tokens which are standardized by guacamole-ext to the given TokenFilter using the values
   * from the given AuthenticatedUser object, including any associated credentials. These
   * standardized tokens include the current username (GUAC_USERNAME), password (GUAC_PASSWORD), and
   * the server date and time (GUAC_DATE and GUAC_TIME respectively). If either the username or
   * password are not set within the given user or their provided credentials, the corresponding
   * token(s) will remain unset.
   *
   * @param filter The TokenFilter to add standard tokens to.
   * @param user   The AuthenticatedUser to use when populating the GUAC_USERNAME and GUAC_PASSWORD
   *               tokens.
   */
