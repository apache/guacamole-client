...
/**
 * The environment of the locally-running Guacamole instance, describing available protocols,
 * configuration parameters, and the GUACAMOLE_HOME directory. Sources of configuration properties
 * like guacamole.properties and environment variables will be automatically added by the Guacamole
 * web application and may also be added by extensions using
...
public class LocalEnvironment implements Environment {
  ...
  /**
   * The hostname to use when connecting to guacd if no hostname is provided within
   * guacamole.properties.
   */
  private static final String DEFAULT_GUACD_HOSTNAME = "localhost";

  /**
   * The port to use when connecting to guacd if no port is provided within guacamole.properties.
   */
  private static final int DEFAULT_GUACD_PORT = 4822;

  /**
   * Whether SSL/TLS is enabled for connections to guacd if not specified within
   * guacamole.properties.
   */
  private static final boolean DEFAULT_GUACD_SSL = false;
  /**
   * All GuacamoleProperties instances added via addGuacamoleProperties(), in the order that they
   * were added. This storage has been made static to allow addGuacamoleProperties() to work as
   * expected even if extensions continue to use the deprecated constructor to create non-singleton
   * instances.
   */
  ...
  /**
   * Singleton instance of this environment, to be returned by calls to getInstance().
   */
  ...
  /**
   * Creates a new LocalEnvironment, initializing that environment based on the contents of
   * GUACAMOLE_HOME. Sources of configuration properties like guacamole.properties and environment
   * variables will be automatically added by the Guacamole web application and/or any installed
   * extensions.
   *
   * @deprecated Extensions leveraging LocalEnvironment should instead use
   * LocalEnvironment.getInstance() to obtain a singleton instance of the environment.
   */
  @Deprecated
  ...
  /**
   * Returns a singleton instance of LocalEnvironment which may be shared by the Guacamole web
   * application and all extensions.
   *
  ...
  /**
   * Locates the Guacamole home directory by checking, in order: the guacamole.home system property,
   * the GUACAMOLE_HOME environment variable, and finally the .guacamole directory in the home
   * directory of the user running the servlet container. If even the .guacamole directory doesn't
   * exist, then /etc/guacamole will be used.
   *
   * @return The File representing the Guacamole home directory, which may or may not exist, and may
   * turn out to not be a directory.
   */
  ...
  /**
   * Parses the given JSON file, returning the parsed ProtocolInfo. The JSON format is conveniently
   * and intentionally identical to a serialized ProtocolInfo object, which is identical to the JSON
   * format used by the protocol REST service built into the Guacamole web application.
   *
   * @param input An input stream containing JSON describing the forms and parameters associated
   *              with a protocol supported by Guacamole.
   * @return A new ProtocolInfo object which contains the forms and parameters described by the JSON
   * file parsed.
   * @throws IOException If an error occurs while parsing the JSON file.
   */
  ...
  /**
   * Reads through all pre-defined protocols and any protocols within the "protocols" subdirectory
   * of GUACAMOLE_HOME, returning a map containing each of these protocols. The key of each entry
   * will be the name of that protocol, as would be passed to guacd during connection.
   *
    ...
    if (protocol_directory.isDirectory()) {
      ...
      for (File file : files) {
        ...
        try {
          ...
          logger.error("Unable to read connection parameter information from \"{}\": {}",
  ...
  /**
   * Returns the string value of the property having the given name, trying each source of
   * properties added with addGuacamoleProperties() until a value is found. If no such property is
   * defined, null is returned.
   *
   * @param name The name of the property value to retrieve.
   * @return The value of the property having the given name, or null if no such property is
   * defined.
   * @throws GuacamoleException If an error occurs retrieving a property from a GuacamoleProperties
