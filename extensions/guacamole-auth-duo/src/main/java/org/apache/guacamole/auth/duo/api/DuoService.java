...
/**
 * Service which produces signed requests and parses/verifies signed responses as required by Duo's
 * API.
 */
public class DuoService {
  ...
  /**
   * Pattern which matches valid Duo responses. Each response is made up of two sections, separated
   * from each other by a colon, where each section is a signed Duo cookie.
   */
  private static final Pattern RESPONSE_FORMAT = Pattern.compile("([^:]+):([^:]+)");

  /**
   * The index of the capturing group within RESPONSE_FORMAT which contains the DUO_RESPONSE cookie
   * signed by the secret key.
   */
  private static final int DUO_COOKIE_GROUP = 1;

  /**
   * The index of the capturing group within RESPONSE_FORMAT which contains the APPLICATION cookie
   * signed by the application key.
   */
  ...
  /**
   * Creates and signs a new request to verify the identity of the given user. This request may
   * ultimately be sent to Duo, resulting in a signed response from Duo if that verification
   * succeeds.
   *
   * @param authenticatedUser The user whose identity should be verified.
   * @return A signed user verification request which can be sent to Duo.
  ...
  /**
   * Returns whether the given signed response is a valid response from Duo which verifies the
   * identity of the given user. If the given response is invalid or does not verify the identity of
   * the given user (including if it is a valid response which verifies the identity of a DIFFERENT
   * user), false is returned.
   *
   * @param authenticatedUser The user that the given signed response should verify.
   * @param signedResponse    The signed response received from Duo in response to a signed
   *                          request.
   * @return true if the signed response is a valid response from Duo AND verifies the identity of
   * the given user, false otherwise.
   * @throws GuacamoleException If required Duo-specific configuration options are missing or
   *                            invalid, or if an error occurs prevents validation of the
   *                            signature.
   */
