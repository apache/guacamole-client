...
/**
 * Service which abstracts the internals of handling SAML requests and responses.
 */
@Singleton
public class SAMLService {
  ...
  /**
   * Creates a new SAML request, beginning the overall authentication flow that will ultimately
   * result in an asserted user identity if the user is successfully authenticated by the SAML IdP.
   * The URI of the SSO endpoint of the SAML IdP that the user must be redirected to for the
   * authentication process to continue is returned.
   *
   * @return The URI of the SSO endpoint of the SAML IdP that the user must be redirected to.
   * @throws GuacamoleException If an error prevents the SAML request and redirect URI from being
   *                            generated.
   */
  ...
  /**
   * Processes the given SAML response, as received by the SAML ACS endpoint at the given URL,
   * producing an {@link AuthenticationSession} that now includes a valid assertion of the user's
   * identity. If the SAML response is invalid in any way, an exception is thrown.
   *
   * @param url             The URL of the ACS endpoint that received the SAML response. This should
   *                        be the URL pointing to the single POST-handling endpoint of
   *                        {@link AssertionConsumerServiceResource}.
   * @param relayState      The "RelayState" value originally provided in the SAML request, which in
   *                        our case is the transient the session identifier of the in-progress
   *                        authentication attempt. The SAML standard requires that the identity
   *                        provider include the "RelayState" value it received alongside its SAML
   *                        response.
   * @param encodedResponse The response received from the SAML IdP via the ACS endpoint at the
   *                        given URL.
   * @return The {@link AuthenticationSession} associated with the in-progress authentication
   * attempt, now associated with the {@link AssertedIdentity} representing the identity of the user
   * asserted by the SAML IdP.
   * @throws GuacamoleException If the given SAML response is not valid, or if the configuration
