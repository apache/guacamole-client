...
/**
 * REST API resource that provides a SAML assertion consumer service (ACS) endpoint. SAML identity
 * providers will issue an HTTP POST to this endpoint asserting the user's identity when the user
...
public class AssertionConsumerServiceResource extends SSOResource {
  ...
  /**
   * Processes the SAML response submitted by the SAML IdP via an HTTP POST. If SSO has been
   * successful, the user is redirected back to Guacamole to complete authentication. If SSO has
   * failed, the user is redirected back to Guacamole to re-attempt authentication.
   *
   ...
   *                        response.
   * @param samlResponse    The encoded response returned by the SAML IdP.
   * @param consumedRequest The HttpServletRequest associated with the SAML response. The parameters
   *                        of this request may not be accessible, as the request may have been
   *                        fully consumed by JAX-RS.
   * @return An HTTP Response that will redirect the user back to Guacamole, with an internal state
   * identifier included in the URL such that the the Guacamole side of authentication can
   * complete.
   * @throws GuacamoleException If configuration information required for processing SAML responses
   *                            cannot be read.
   */
  @POST
  @Path("callback")
