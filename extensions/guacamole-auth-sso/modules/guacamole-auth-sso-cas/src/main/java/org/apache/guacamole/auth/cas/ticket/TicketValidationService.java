...
/**
 * Service for validating ID tickets forwarded to us by the client, verifying that they did indeed
 * come from the CAS service.
 */
public class TicketValidationService {
  ...
  /**
   * Converts the given CAS attribute value object (whose type is variable) to a Set of String
   * values. If the value is already a Collection of some kind, its values are converted to Strings
   * and returned as the members of the Set. If the value is not already a Collection, it is assumed
   * to be a single value, converted to a String, and used as the sole member of the set.
   *
   * @param obj The CAS attribute value to convert to a Set of Strings.
   * @return A Set of all String values contained within the given CAS attribute value.
   */
  ...
  /**
   * Validates and parses the given ID ticket, returning a map of all available tokens for the given
   * user based on attributes provided by the CAS server.  If the ticket is invalid an exception is
   * thrown.
   *
   * @param ticket      The ID ticket to validate and parse.
   * @param credentials The Credentials object to store retrieved username and password values in.
   * @return A CASAuthenticatedUser instance containing the ticket data returned by the CAS server.
   * @throws GuacamoleException If the ID ticket is not valid or guacamole.properties could not be
   *                            parsed.
   */
  ...
  /**
   * Takes an encrypted string representing a password provided by the CAS ClearPass service and
   * decrypts it using the private key configured for this extension.  Returns null if it is unable
  ...
  private final String decryptPassword(String encryptedPassword)
    ...
    try {
      ...
      throw new GuacamoleServerException(
          "Specified private key for ClearPass decryption is invalid.", e);
    } catch (NoSuchAlgorithmException e) {
      throw new GuacamoleServerException("Unexpected algorithm for the private key.", e);
    } catch (NoSuchPaddingException e) {
      throw new GuacamoleServerException(
          "No such padding trying to initialize cipher with private key.", e);
    }
