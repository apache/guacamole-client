...
/**
 * Service for validating ID tokens forwarded to us by the client, verifying that they did indeed
 * come from the OpenID service.
 */
public class TokenValidationService {
  ...
  /**
   * Validates the given ID token, returning the JwtClaims contained therein. If the ID token is
   * invalid, null is returned.
   *
   * @param token The ID token to validate.
   * @return The JWT claims contained within the given ID token if it passes tests, or null if the
   * token is not valid.
   * @throws GuacamoleException If guacamole.properties could not be parsed.
   */
  ...
  /**
   * Parses the given JwtClaims, returning the username contained therein, as defined by the
   * username claim type given in guacamole.properties. If the username claim type is missing or is
   * invalid, null is returned.
   *
   * @param claims A valid JwtClaims to extract the username from.
   * @return The username contained within the given JwtClaims, or null if the claim is not valid or
   * the username claim type is missing,
   * @throws GuacamoleException If guacamole.properties could not be parsed.
   */
  ...
  /**
   * Parses the given JwtClaims, returning the groups contained therein, as defined by the groups
   * claim type given in guacamole.properties. If the groups claim type is missing or is invalid, an
   * empty set is returned.
   *
   * @param claims A valid JwtClaims to extract groups from.
   * @return A Set of String representing the groups the user is member of from the OpenID provider
   * point of view, or an empty Set if claim is not valid or the groups claim type is missing,
   * @throws GuacamoleException If guacamole.properties could not be parsed.
   */
