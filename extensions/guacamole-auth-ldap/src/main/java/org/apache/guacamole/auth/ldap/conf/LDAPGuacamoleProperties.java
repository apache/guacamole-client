...
/**
 * Provides properties required for use of the LDAP authentication provider. These properties will
 * be read from guacamole.properties when the LDAP authentication provider is used.
 */
public class LDAPGuacamoleProperties {
  ...
  public static final LdapDnGuacamoleProperty LDAP_CONFIG_BASE_DN =
      new LdapDnGuacamoleProperty() {
        ...
        public String getName() {
          return "ldap-config-base-dn";
        }

      };
  /**
   * The base DN of users. All users must be contained somewhere within the subtree of this DN. If
   * the LDAP authentication will not be given its own credentials for querying other LDAP users,
   * all users must be direct children of this base DN, varying only by LDAP_USERNAME_ATTRIBUTE.
   */
  public static final LdapDnGuacamoleProperty LDAP_USER_BASE_DN =
      new LdapDnGuacamoleProperty() {
        ...
        @Override
        public String getName() {
          return "ldap-user-base-dn";
        }

      };
  /**
   * The base DN of role based access control (RBAC) groups. All groups which will be used for RBAC
   * must be contained somewhere within the subtree of this DN.
   */
  public static final LdapDnGuacamoleProperty LDAP_GROUP_BASE_DN =
      new LdapDnGuacamoleProperty() {
        ...
        @Override
        public String getName() {
          return "ldap-group-base-dn";
        }

      };
  /**
   * The attribute or attributes which identify users. One of these attributes must be present
   * within each Guacamole user's record in the LDAP directory. If the LDAP authentication will not
   * be given its own credentials for querying other LDAP users, this list may contain only one
   * attribute, and the concatenation of that attribute and the value of LDAP_USER_BASE_DN must
   * equal the user's full DN.
   */
  ...
  /**
   * The attribute or attributes which identify user groups. One of these attributes must be present
   * within each Guacamole user group's record in the LDAP directory for that group to be visible.
   */
  public static final StringListProperty LDAP_GROUP_NAME_ATTRIBUTE =
      new StringListProperty() {
        ...
        @Override
        public String getName() {
          return "ldap-group-name-attribute";
        }
   ...
   * attempting to log in. The format of this parameter will vary based on the LDAP server
   * implementation - often it is expected to be in full LDAP DN format; however various LDAP server
   * implementations allow this to be in other formats (e.g. Active Directory allows User Principal
   * Name, or UPN, format). For this reason the configuration allows this to be any string.
   */
  public static final StringGuacamoleProperty LDAP_SEARCH_BIND_DN =
      new StringGuacamoleProperty() {
        ...
        @Override
        public String getName() {
          return "ldap-search-bind-dn";
        }

      };
  /**
   * The password to provide to the LDAP server when binding as LDAP_SEARCH_BIND_DN. If
   * LDAP_SEARCH_BIND_DN is not specified, this property has no effect. If this property is not
   * specified, no password will be provided when attempting to bind as LDAP_SEARCH_BIND_DN.
   */
  public static final StringGuacamoleProperty LDAP_SEARCH_BIND_PASSWORD =
      new StringGuacamoleProperty() {
        ...
        @Override
        public String getName() {
          return "ldap-search-bind-password";
        }

      };
  /**
   * The encryption method to use when connecting to the LDAP server, if any. The chosen method will
   * also dictate the default port if not already explicitly specified via LDAP_PORT.
   */
  ...
  /**
   * Property that controls whether or not the LDAP connection follows (dereferences) aliases as it
   * searches the tree.
   */
  public static final EnumGuacamoleProperty<AliasDerefMode> LDAP_DEREFERENCE_ALIASES =
      new EnumGuacamoleProperty<AliasDerefMode>(
        ...
        @Override
        public String getName() {
          return "ldap-dereference-aliases";
        }
  ...
  public static final LdapFilterGuacamoleProperty LDAP_GROUP_SEARCH_FILTER =
      new LdapFilterGuacamoleProperty() {
        ...
        @Override
        public String getName() {
          return "ldap-group-search-filter";
        }
  ...
  public static final EnumGuacamoleProperty<MemberAttributeType> LDAP_MEMBER_ATTRIBUTE_TYPE =
      new EnumGuacamoleProperty<MemberAttributeType>(MemberAttributeType.class) {
        ...
        @Override
        public String getName() {
          return "ldap-member-attribute-type";
        }
