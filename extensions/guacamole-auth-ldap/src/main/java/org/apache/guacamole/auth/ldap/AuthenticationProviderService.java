...
/**
 * Service providing convenience functions for the LDAP AuthenticationProvider implementation.
 */
public class AuthenticationProviderService {
  ...
  /**
   * Determines the DN which corresponds to the user having the given username. The DN will either
   * be derived directly from the user base DN, or queried from the LDAP server, depending on how
   * LDAP authentication has been configured.
   *
   * @param config   The configuration of the LDAP server being queried.
   * @param username The username of the user whose corresponding DN should be returned.
   * @return The DN which corresponds to the user having the given username.
   * @throws GuacamoleException If required properties are missing, and thus the user DN cannot be
   *                            determined.
   */
  ...
  /**
   * Returns a new UserLDAPConfiguration that is connected to an LDAP server associated with the
   * Guacamole user having the given username and bound using the provided password. All LDAP
   * servers associated with the given user are tried until the connection and authentication
   * attempt succeeds. If no LDAP servers are available, or no LDAP servers are associated with the
   * given user, null is returned. The Guacamole username will be internally translated to a
   * fully-qualified LDAP DN according to the configuration of the LDAP server that is ultimately
   * chosen.
   *
   * @param username The username of the Guacamole user to bind as.
   * @param password The password of the user to bind as.
   * @return A new UserLDAPConfiguration which is bound to an LDAP server using the provided
   * credentials, or null if no LDAP servers are available for the given user or
   * connecting/authenticating has failed.
   * @throws GuacamoleException If configuration information for the user's LDAP server(s) cannot be
   *                            retrieved.
   */
  private UserLDAPConfiguration getLDAPConfiguration(String username,
    ...
    for (LDAPConfiguration config : configs) {
      ...
      if (translatedUsername == null) {
        logger.debug("LDAP server \"{}\" does not match username \"{}\".",
            config.getServerHostname(), username);
        continue;
      }
      ...
      logger.info("User \"{}\" was successfully authenticated by LDAP server \"{}\".", username,
  ...
  /**
   * Returns an AuthenticatedUser representing the user authenticated by the given credentials. Also
   * adds custom LDAP attributes to the AuthenticatedUser.
   *
  ...
  /**
   * Returns parameter tokens generated from LDAP attributes on the user currently bound under the
   * given LDAP connection. The attributes to be converted into parameter tokens must be explicitly
   * listed in guacamole.properties. If no attributes are specified or none are found on the LDAP
   * user object, an empty map is returned.
   *
   * @param config The configuration of the LDAP server being queried.
   * @return A map of parameter tokens generated from attributes on the user currently bound under
   * the given LDAP connection, as a map of token name to corresponding value, or an empty map if no
   * attributes are specified or none are found on the user object.
   * @throws GuacamoleException If an error occurs retrieving the user DN or the attributes.
   */
