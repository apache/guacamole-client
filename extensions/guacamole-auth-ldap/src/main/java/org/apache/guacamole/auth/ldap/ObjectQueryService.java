...
/**
 * Service for executing queries against an LDAP directory intended to retrieve Guacamole-related
 * objects. Referrals are automatically handled. Convenience functions are provided for generating
 * the LDAP queries typically required for retrieving Guacamole objects, as well as for converting
 * the results of a query into a {@link Map} of Guacamole objects.
 */
public class ObjectQueryService {
  ...
  /**
   * Returns the identifier of the object represented by the given LDAP entry. Multiple attributes
   * may be declared as containing the identifier of the object when present on an LDAP entry. If
   * multiple such attributes are present on the same LDAP entry, the value of the attribute with
   * highest priority is used. If multiple copies of the same attribute are present on the same
   * LDAPentry, the first value of that attribute is used.
   *
   * @param entry      The entry representing the Guacamole object whose unique identifier should be
   *                   determined.
   * @param attributes A collection of all attributes which may be used to specify the unique
   *                   identifier of the Guacamole object represented by an LDAP entry, in order of
   *                   decreasing priority.
   * @return The identifier of the object represented by the given LDAP entry, or null if no
   * attributes declared as containing the identifier of the object are present on the entry.
   * @throws LdapInvalidAttributeValueException If an error occurs retrieving the value of the
  ...
  /**
   * Generates a properly-escaped LDAP query which finds all objects which match the given LDAP
   * filter and which have at least one of the given attributes set to the specified value.
   *
   * @param filter         The LDAP filter to apply to reduce the results of the query in addition
   *                       to testing the values of the given attributes.
   * @param attributes     A collection of all attributes to test for equivalence to the given
   *                       value, in order of decreasing priority.
   * @param attributeValue The value that the resulting LDAP query should search for within the
   *                       attributes of objects within the LDAP directory. If null, the resulting
   *                       LDAP query will search for the presence of at least one of the given
   *                       attributes on each object, regardless of the value of those attributes.
   * @return An LDAP query which will search for arbitrary LDAP objects having at least one of the
   * given attributes set to the specified value.
   */
  ...
  /**
   * Executes an arbitrary LDAP query using the given connection, returning a list of all results.
   * Only objects beneath the given base DN are included in the search.
   *
   * @param config         The configuration of the LDAP server being queried.
   * @param ldapConnection The current connection to the LDAP server, associated with the current
   *                       user.
   * @param baseDN         The base DN to search using the given LDAP query.
   * @param query          The LDAP query to execute.
   * @param searchHop      The current level of referral depth for this search, used for limiting
   *                       the maximum depth to which referrals can go.
   * @param attributes     A collection of the names of attributes that should be retrieved from
   *                       LDAP entries returned by the search, or null if all available attributes
   *                       should be returned.
   * @return A list of all results accessible to the user currently bound under the given LDAP
   * connection.
   * @throws GuacamoleException If an error occurs executing the query, or if configuration
  ...
  public List<Entry> search(LDAPConfiguration config,
    ...
    if (searchHop >= maxHops) {
      logger.debug("Refusing to follow further referrals as the maximum "
              ...
              + "property to a larger value.", maxHops,
          LDAPGuacamoleProperties.LDAP_MAX_REFERRAL_HOPS.getName());
        ...
        // following is enabled
          ...
          if (request.isFollowReferrals()) {
            for (String url : results.getReferral().getLdapUrls()) {
              ...
              try (LdapNetworkConnection referralConnection = ldapService.bindAs(config, url,
                  ldapConnection)) {
                if (referralConnection != null) {
                  logger.debug("Following referral to \"{}\"...", url);
                  entries.addAll(
                      search(config, referralConnection, baseDN, query, searchHop + 1, attributes));
                } else {
  ...
  /**
   * Executes the query which would be returned by generateQuery() using the given connection,
   * returning a list of all results. Only objects beneath the given base DN are included in the
   * search.
   *
   * @param config           The configuration of the LDAP server being queried.
   * @param ldapConnection   The current connection to the LDAP server, associated with the current
   ...
   * @param filter           The LDAP filter to apply to reduce the results of the query in addition
   *                         to testing the values of the given attributes.
   * @param filterAttributes A collection of all attributes to test for equivalence to the given
   *                         value, in order of decreasing priority.
   * @param filterValue      The value that should be searched search for within the attributes of
   *                         objects within the LDAP directory. If null, the search will test only
   *                         for the presence of at least one of the given attributes on each
   *                         object, regardless of the value of those attributes.
   * @param attributes       A collection of the names of attributes that should be retrieved from
   *                         LDAP entries returned by the search, or null if all available
  ...
  /**
   * Converts a given list of LDAP entries to a {@link Map} of Guacamole objects stored by their
   * identifiers.
   *
   * @param <ObjectType> The type of object to store within the {@link Map}.
   * @param entries      A list of LDAP entries to convert to Guacamole objects.
   * @param mapper       A mapping function which converts a given LDAP entry to its corresponding
   *                     Guacamole object. If the LDAP entry cannot be converted, null should be
   *                     returned.
   * @return A new {@link Map} containing Guacamole object versions of each of the given LDAP
   * entries, where each object is stored within the {@link Map} under its corresponding
   * identifier.
   */
