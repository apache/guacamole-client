...
public class ConfigurationService {
  ...
  /**
   * The name of the file within GUACAMOLE_HOME that defines each available LDAP server (if not
  ...
  private final ObjectMapper mapper = new ObjectMapper(new YAMLFactory())
      .registerModule(new SimpleModule().addDeserializer(Pattern.class,
          new CaseInsensitivePatternDeserializer()));
  /**
   * The timestamp that the {@link #LDAP_SERVERS_YML} was last modified when it was read, as would
   * be returned by {@link File#lastModified()}.
   */
  private final AtomicLong lastModified = new AtomicLong(0);

  /**
   * The cached copy of the configuration read from {@link #LDAP_SERVERS_YML}. If the current set of
   * LDAP servers has not yet been read from the YAML configuration file, or if guacamole.properties
   * is being used instead, this will be null.
   */
  ...
  /**
   * Returns the configuration information for all configured LDAP servers. If multiple servers are
   * returned, each should be tried in order until a successful LDAP connection is established.
   *
   * @return The configurations of all LDAP servers.
   * @throws GuacamoleException If the configuration information of the LDAP servers cannot be
  ...
  public Collection<? extends LDAPConfiguration> getLDAPConfigurations() throws GuacamoleException {
    ...
    if (ldapServers.exists()) {
      ...
      if (currentLastModified > oldLastModified && lastModified.compareAndSet(oldLastModified,
          currentLastModified)) {
        try {

          logger.debug("Reading updated LDAP configuration from \"{}\"...", ldapServers);
          Collection<JacksonLDAPConfiguration> configs = mapper.readValue(ldapServers,
              new TypeReference<Collection<JacksonLDAPConfiguration>>() {
              });
    ...
    // Clear cached YAML if it no longer exists
      ...
      if (lastModified.compareAndSet(oldLastModified, 0)) {
        logger.debug("Clearing cached LDAP configuration from \"{}\" (file no longer exists).",
            ldapServers);
