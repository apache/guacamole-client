...
public class RadiusConnectionService {
   ...
   *
   * @return A RadiusClient instance, configured with server, shared secret, ports, and timeout, as
  ...
  /**
   * Creates a new instance of RadiusAuthenticator, configured with parameters specified within
   * guacamole.properties.
   *
   * @return A new RadiusAuthenticator instance which has been configured with parameters from
   * guacamole.properties.
   * @throws GuacamoleException If the configuration cannot be read or the inner protocol is not
   *                            configured when the client is set up for a tunneled RADIUS
   *                            connection.
   */
   ...
   * @param secret        The secret, usually a password or challenge response, to send to
   *                      authenticate to the RADIUS server.
   * @param clientAddress The IP address of the client, if known, which will be set in as the RADIUS
   *                      client address.
   * @param state         The previous state of the RADIUS connection
   * @return A RadiusPacket with the response of the server.
   * @throws GuacamoleException If an error occurs while talking to the server.
   */
  ...
  /**
   * Send a challenge response to the RADIUS server by validating the input and then sending it
   * along to the authenticate method.
   *
   * @param username      The username to send to the RADIUS server for authentication.
   * @param response      The response phrase to send to the RADIUS server in response to the
   *                      challenge previously provided.
   * @param clientAddress The IP address of the client, if known, which will be set in as the RADIUS
   *                      client address.
   * @param state         The state data provided by the RADIUS server in order to continue the
   *                      RADIUS conversation.
   * @return A RadiusPacket containing the server's response to the authentication attempt.
   * @throws GuacamoleException If an error is encountered trying to talk to the RADIUS server.
   */
