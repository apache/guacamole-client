...
/**
 * Service providing convenience functions for the RADIUS AuthenticationProvider implementation.
 */
public class AuthenticationProviderService {

  /**
   * The name of the password field where the user will enter a response to the RADIUS challenge.
   */
  ...
  /**
   * Returns an object containing the challenge message and the expected credentials from a RADIUS
   * challenge, or null if either state or reply attributes are missing from the challenge.
   *
   * @param challengePacket The AccessChallenge RadiusPacket received from the RADIUS server.
   * @return A GuacamoleRadiusChallenge object that contains the challenge message sent by the
   * RADIUS server and the expected credentials that should be requested of the user in order to
   * continue authentication.  One of the expected credentials *must* be the RADIUS state.  If
   * either state or the reply are missing from the challenge this method will return null.
   */
  ...
  public AuthenticatedUser authenticateUser(Credentials credentials)
    ...
    if (challengeResponse == null) {
      ...
      try {
        ...
        throw new GuacamoleInvalidCredentialsException("Authentication error.",
    ...
    // This is a response to a previous challenge, authenticate with that.
      ...
      try {
        ...
        logger.warn("Illegal hexadecimal value while parsing RADIUS state string: {}",
            e.getMessage());
        logger.debug("Encountered exception while attempting to parse the hexidecimal state value.",
            e);
        throw new GuacamoleInvalidCredentialsException("Authentication error.",
            CredentialsInfo.USERNAME_PASSWORD);
      } catch (GuacamoleException e) {
    ...
    // Something unanticipated happened, so panic and go back to login.
      ...
      throw new GuacamoleInvalidCredentialsException("Unknown error trying to authenticate.",
          CredentialsInfo.USERNAME_PASSWORD);
    }
