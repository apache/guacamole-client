 ...
 * connecting to a SQLite database based on SQLite-specific options
 * provided via guacamole.properties.
 */
public class SQLiteInjectorProvider extends JDBCInjectorProvider {

    @Override
    protected Injector create() throws GuacamoleException {

        // Get local environment
        SQLiteEnvironment environment = new SQLiteEnvironment();

        // Set up Guice injector
        return Guice.createInjector(
            new JDBCAuthenticationProviderModule(environment),
            new SQLiteAuthenticationProviderModule(environment)
        );
