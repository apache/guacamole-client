...
/**
 * GuacamoleTunnelService implementation which restricts concurrency for each connection and group
 * according to a maximum number of connections and maximum number of connections per user.
 */
@Singleton
public class RestrictedGuacamoleTunnelService
  ...
  /**
   * Attempts to add a single instance of the given value to the given multiset without exceeding
   * the specified maximum number of values. If the value cannot be added without exceeding the
   * maximum, false is returned.
   *
   ...
   * @param max      The maximum number of each distinct value that the given multiset should hold,
   *                 or zero if no limit applies.
   * @return true if the value was successfully added without exceeding the specified maximum, false
  ...
  /**
   * Attempts to increment the given AtomicInteger without exceeding the specified maximum value. If
   * the AtomicInteger cannot be incremented without exceeding the maximum, false is returned.
   *
   * @param counter The AtomicInteger to attempt to increment.
   * @param max     The maximum value that the given AtomicInteger should contain, or zero if no
   *                limit applies.
   * @return true if the AtomicInteger was successfully incremented without exceeding the specified
   * maximum, false if the AtomicInteger could not be incremented.
   */
  ...
  protected ModeledConnection acquire(RemoteAuthenticatedUser user,
    ...
    ModeledConnection[] sortedConnections = connections.toArray(
        new ModeledConnection[connections.size()]);
