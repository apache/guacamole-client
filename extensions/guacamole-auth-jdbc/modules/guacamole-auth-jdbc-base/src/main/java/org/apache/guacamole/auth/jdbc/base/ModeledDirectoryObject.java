 ...
 * and are persisted to an underlying database model. All such objects will need the same base set
 * of queries to fulfill the needs of the Directory class.
 *
...
public abstract class ModeledDirectoryObject<ModelType extends ObjectModel>
  ...
  /**
   * Returns the names of all attributes explicitly supported by this object. Attributes named here
   * have associated mappings within the backing model object, and thus should not be included in
   * the arbitrary attribute storage. Any attributes set which do not match these names, such as
   * those set via other extensions, will be added to arbitrary attribute storage.
   *
   * @return A read-only Set of the names of all attributes explicitly supported (mapped to a
   * property of the backing model) by this object.
   */
