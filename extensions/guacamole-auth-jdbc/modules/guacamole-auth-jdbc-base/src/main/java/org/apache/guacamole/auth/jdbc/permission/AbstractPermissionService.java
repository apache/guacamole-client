...
/**
 * Abstract PermissionService implementation which provides additional convenience methods for
 * enforcing the permission model.
 *
...
public abstract class AbstractPermissionService<PermissionSetType extends PermissionSet<PermissionType>,
  ...
  /**
   * Returns the ObjectPermissionSet related to the type of the given entity. If the given entity
   * represents a user, then the ObjectPermissionSet containing user permissions is returned. If the
   * given entity represents a user group, then the ObjectPermissionSet containing user group
   * permissions is returned.
   *
   * @param user         The user to retrieve the ObjectPermissionSet from.
   * @param targetEntity The entity whose type dictates the ObjectPermissionSet returned.
   * @return The ObjectPermissionSet related to the type of the given entity.
   * @throws GuacamoleException If the relevant ObjectPermissionSet cannot be retrieved.
   */
  ...
  /**
   * Determines whether the given user can read the permissions currently granted to the given
   * target entity. If the reading user and the target entity are not the same, then explicit READ
   * or SYSTEM_ADMINISTER access is required. Permission inheritance via user groups is taken into
   * account.
   *
   * @param user         The user attempting to read permissions.
   * @param targetEntity The entity whose permissions are being read.
   * @return true if permission is granted, false otherwise.
   * @throws GuacamoleException If an error occurs while checking permission status, or if
