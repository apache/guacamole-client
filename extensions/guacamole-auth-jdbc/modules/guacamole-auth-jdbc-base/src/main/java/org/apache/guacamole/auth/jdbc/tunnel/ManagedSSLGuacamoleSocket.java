...
/**
 * Implementation of GuacamoleSocket which connects via SSL to a given hostname and port. If the
 * socket is closed for any reason, a given task is run.
 */
public class ManagedSSLGuacamoleSocket extends SSLGuacamoleSocket {
  ...
  /**
   * Creates a new socket which connects via SSL to a given hostname and port. If the socket is
   * closed for any reason, the given task is run.
   *
   ...
   * @param socketClosedTask The task to run when the socket is closed. This task will NOT be run if
   *                         an exception occurs during connection, and this
