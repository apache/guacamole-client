...
/**
 * Pooled DataSource implementation which dynamically retrieves the database username and password
 * from the Guacamole server environment each time a new database connection is created.
 */
@Singleton
public class DynamicallyAuthenticatedDataSource extends PooledDataSource {
  ...
  private static final Logger logger = LoggerFactory.getLogger(
      DynamicallyAuthenticatedDataSource.class);

  /**
   * Creates a new DynamicallyAuthenticatedDataSource which dynamically retrieves database
   * credentials from the given JDBCEnvironment each time a new database connection is needed.
   *
   * @param environment       The JDBCEnvironment that should be used to retrieve database
   *                          credentials.
