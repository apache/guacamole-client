...
public class ConnectionSharingService {
  ...
  /**
   * The name of the query parameter that is used when authenticating obtain temporary access to a
   * connection.
   */
  public static final String SHARE_KEY_NAME = "key";
  /**
   * The credentials expected when a user is authenticating using temporary credentials in order to
   * obtain access to a single connection.
   */
  ...
  /**
   * Creates a new SharedConnectionDefinition which can be used to connect to the given connection,
   * optionally restricting access to the shared connection using the given sharing profile. If the
   * user does not have permission to share the connection via the given sharing profile, permission
   * will be denied.
   *
   * @param user                     The user sharing the connection.
   * @param activeConnection         The active connection being shared.
   * @param sharingProfileIdentifier The identifier of the sharing profile dictating the semantics
   *                                 or restrictions applying to the shared session, or null if no
   *                                 such restrictions should apply.
   * @return A new SharedConnectionDefinition which can be used to connect to the given connection.
   * @throws GuacamoleException If permission to share the given connection is denied.
  ...
  public SharedConnectionDefinition shareConnection(ModeledAuthenticatedUser user,
    ...
    if (sharingProfileIdentifier != null) {
      ...
      if (sharingProfile == null || !sharingProfile.getPrimaryConnectionIdentifier()
          .equals(connectionIdentifier)) {
    ...
    SharedConnectionDefinition definition = new SharedConnectionDefinition(activeConnection,
        sharingProfile, key);
  ...
  /**
   * Generates a set of temporary credentials which can be used to connect to the given connection
   * shared by the SharedConnectionDefinition.
   *
   * @param definition The SharedConnectionDefinition which defines the connection being shared and
   *                   any applicable restrictions.
   * @return A newly-generated set of temporary credentials which can be used to connect to the
   * connection shared by the given SharedConnectionDefinition.
   */
  ...
  /**
   * Returns the share key contained within the given credentials. If there is no such share key,
   * null is returned.
   *
   * @param credentials The credentials from which the share key should be retrieved.
   * @return The share key contained within the given credentials, or null if the credentials do not
   * contain a share key.
   */
  ...
  /**
   * Returns a SharedAuthenticatedUser if the given credentials contain a valid share key. The
   * returned user will be associated with the single shared connection to which they have been
   * granted temporary access. If the share key is invalid, or no share key is contained within the
   * given credentials, null is returned.
   *
   * @param authProvider The AuthenticationProvider on behalf of which the user is being retrieved.
   * @param credentials  The credentials which are expected to contain the share key.
   * @return A SharedAuthenticatedUser with access to a single shared connection, if the share key
   * within the given credentials is valid, or null if the share key is invalid or absent.
   */
