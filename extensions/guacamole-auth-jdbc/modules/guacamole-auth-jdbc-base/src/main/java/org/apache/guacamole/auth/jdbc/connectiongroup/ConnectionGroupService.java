...
/**
 * Service which provides convenience methods for creating, retrieving, and manipulating connection
 * groups.
 */
public class ConnectionGroupService extends
    ModeledChildDirectoryObjectService<ModeledConnectionGroup,
  ...
  protected void beforeCreate(ModeledAuthenticatedUser user,
    ...
    ConnectionGroupModel existing = connectionGroupMapper.selectOneByName(
        model.getParentIdentifier(), model.getName());
    if (existing != null) {
      throw new GuacamoleClientException(
          "The connection group \"" + model.getName() + "\" already exists.");
    }
  ...
  /**
   * Returns the set of all identifiers for all connection groups within the connection group having
   * the given identifier. Only connection groups that the user has read access to will be
   * returned.
   * <p>
   ...
   *                   connection group.
   * @return The set of all identifiers for all connection groups in the connection group having the
   * given identifier that the user has read access to.
  ...
  /**
   * Connects to the given connection group as the given user, using the given client information.
   * If the user does not have permission to read the connection group, permission will be denied.
   *
   * @param user            The user connecting to the connection group.
   * @param connectionGroup The connectionGroup being connected to.
