...
/**
 * Base implementation of the GuacamoleTunnelService, handling retrieval of connection parameters,
 * load balancing, and connection usage counts. The implementation of concurrency rules is up to
...
public abstract class AbstractGuacamoleTunnelService implements GuacamoleTunnelService {
  ...
  /**
   * Acquires possibly-exclusive access to any one of the given connections on behalf of the given
   * user. If access is denied for any reason, or if no connection is available, an exception is
   * thrown.
   *
   * @param user                The user acquiring access.
   * @param connections         The connections being accessed.
   * @param includeFailoverOnly Whether connections which have been designated for use in failover
   *                            situations only (hot spares) may be considered.
   * @return The connection that has been acquired on behalf of the given user.
   * @throws GuacamoleException If access is denied to the given user for any reason.
   */
  ...
  /**
   * Releases possibly-exclusive access to the given connection on behalf of the given user. If the
   * given user did not already have access, the behavior of this function is undefined.
   *
   ...
   * If access is denied for any reason, an exception is thrown.
   *
   * @param user            The user acquiring access.
   * @param connectionGroup The connection group being accessed.
   * @throws GuacamoleException If access is denied to the given user for any reason.
   */
   ...
   * If the given user did not already have access, the behavior of this function is undefined.
   *
   * @param user            The user releasing access.
   * @param connectionGroup The connection group being released.
   */
  protected abstract void release(RemoteAuthenticatedUser user,
      ModeledConnectionGroup connectionGroup);

  /**
   * Returns a GuacamoleConfiguration which connects to the given connection. If the ID of an active
   * connection is provided, that active connection will be joined rather than establishing an
   * entirely new connection. If a sharing profile is provided, the parameters associated with that
   * sharing profile will be used to define the access provided to the user accessing the shared
   * connection.
   *
   * @param connection     The connection that the user is connecting to.
   * @param connectionID   The ID of the active connection being joined, as provided by guacd when
   *                       the original connection was established, or null if a new connection
   *                       should be established instead.
   * @param sharingProfile The sharing profile whose associated parameters dictate the level of
   *                       access granted to the user joining the connection, or null if the
   *                       parameters associated with the connection should be used.
   * @return A GuacamoleConfiguration defining the requested, possibly shared connection.
   */
  private GuacamoleConfiguration getGuacamoleConfiguration(
    ...
    if (sharingProfile != null) {
      Collection<SharingProfileParameterModel> parameters = sharingProfileParameterMapper.select(
          sharingProfile.getIdentifier());
      for (SharingProfileParameterModel parameter : parameters) {
        config.setParameter(parameter.getName(), parameter.getValue());
      }
    } else {
      Collection<ConnectionParameterModel> parameters = connectionParameterMapper.select(
  ...
  /**
   * Returns an unconfigured GuacamoleSocket that is already connected to guacd as specified in
   * guacamole.properties, using SSL if necessary.
   *
   * @param proxyConfig          The configuration information to use when connecting to guacd.
   * @param socketClosedCallback The callback which should be invoked whenever the returned socket
   *                             closes.
   * @return An unconfigured GuacamoleSocket, already connected to guacd.
   * @throws GuacamoleException If an error occurs while connecting to guacd, or while parsing
  ...
  /**
   * Creates a tunnel for the given user which connects to the given connection, which MUST already
   * be acquired via acquire(). The given client information will be passed to guacd when the
   * connection is established.
   * <p>
   * The connection will be automatically released when it closes, or if it fails to establish
   * entirely.
   *
   * @param activeConnection The active connection record of the connection in use.
   * @param info             Information describing the Guacamole client connecting to the given
   *                         connection.
   * @param tokens           A Map containing the token names and corresponding values to be applied
   *                         as parameter tokens when establishing the connection.
   * @param interceptErrors  Whether errors from the upstream remote desktop should be intercepted
   *                         and rethrown as GuacamoleUpstreamExceptions.
   * @return A new GuacamoleTunnel which is configured and connected to the given connection.
   * @throws GuacamoleException If an error occurs while the connection is being established, or
  ...
  private GuacamoleTunnel assignGuacamoleTunnel(ActiveConnectionRecord activeConnection,
    ...
    try {
      connectionRecordMapper.insert(
          activeConnection.getModel()); // This MUST happen before getUUID() is invoked, to ensure the ID driving the UUID exists
    ...
    try {
      ...
      // a sharing profile, generate a configuration which does so
        ...
        config = getGuacamoleConfiguration(connection, connectionID,
            activeConnection.getSharingProfile());
  ...
  /**
   * Filters the given collection of connection identifiers, returning a new collection which
   * contains only those identifiers which are preferred. If no connection identifiers within the
   * given collection are preferred, the collection is left untouched.
   *
   * @param user        The user whose preferred connections should be used to filter the given
   *                    collection of connection identifiers.
   * @param identifiers The collection of connection identifiers that should be filtered.
   * @return A collection of connection identifiers containing only the subset of connection
   * identifiers which are also preferred or, if none of the provided identifiers are preferred, the
   * original collection of identifiers.
   */
  ...
  /**
   * Returns a list of all balanced connections within a given connection group. If the connection
   * group is not balancing, or it contains no connections, an empty list is returned.
   *
   * @param user            The user on whose behalf the balanced connections within the given
   *                        connection group are being retrieved.
   * @param connectionGroup The connection group to retrieve the balanced connections of.
   * @return A list containing all balanced connections within the given group, or an empty list if
  ...
  private List<ModeledConnection> getBalancedConnections(ModeledAuthenticatedUser user,
    ...
    Collection<String> identifiers = connectionMapper.selectIdentifiersWithin(
  ...
  public GuacamoleTunnel getGuacamoleTunnel(final ModeledAuthenticatedUser user,
    ...
    ActiveConnectionRecord connectionRecord = new ActiveConnectionRecord(connectionMap, user,
        connection);
  ...
  public GuacamoleTunnel getGuacamoleTunnel(ModeledAuthenticatedUser user,
    ...
    do {
      ...
      // connections
        ...
        logger.info(
            "Upstream error intercepted for connection \"{}\". Failing over to next connection in group...",
            connection.getIdentifier());
    ...
    throw new GuacamoleResourceConflictException(
        "Cannot connect. All upstream connections are unavailable.");
  ...
  /**
   * Task which handles cleanup of a connection associated with some given ActiveConnectionRecord.
   */
  private class ConnectionCleanupTask implements Runnable {
    ...
    /**
     * The ActiveConnectionRecord whose connection will be cleaned up once this task runs.
     */
    private final ActiveConnectionRecord activeConnection;

    /**
     * Creates a new task which automatically cleans up after the connection associated with the
     * given ActiveConnectionRecord. The connection and parent group will be removed from the maps
     * of active connections and groups, and exclusive access will be released.
     *
     * @param activeConnection The ActiveConnectionRecord whose associated connection should be
