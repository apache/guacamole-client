...
/**
 * Connection implementation that creates a history record when the connection is established, and
 * returns a HistoryTrackingTunnel to automatically set the end date when the connection is closed.
 */
public class HistoryTrackingConnection extends DelegatingConnection {
  ...
  /**
   * The connection record mapper to use when writing history entries for established connections.
   */
  private final ConnectionRecordMapper connectionRecordMapper;

  /**
   * Creates a new HistoryConnection that wraps the given connection, automatically creating a
   * history record when the connection is established, and returning a HistoryTrackingTunnel to set
   * the end date on the history entry when the connection is closed.
   *
   * @param currentUser            The current Guacamole user.
   * @param remoteHost             The remote host that the user connected from.
   * @param connection             The connection to wrap.
   * @param connectionRecordMapper The connection record mapper that will be used to write the
   *                               connection history records.
   */
  public HistoryTrackingConnection(User currentUser, String remoteHost, Connection connection,
