...
/**
 * Service which provides convenience methods for creating, retrieving, and manipulating sharing
 * profiles.
 */
public class SharingProfileService
  ...
  protected void beforeCreate(ModeledAuthenticatedUser user,
    ...
    SharingProfileModel existing = sharingProfileMapper.selectOneByName(model.getParentIdentifier(),
        model.getName());
    if (existing != null) {
      throw new GuacamoleClientException(
          "The sharing profile \"" + model.getName() + "\" already exists.");
    }
  ...
  /**
   * Given an arbitrary Guacamole sharing profile, produces a collection of parameter model objects
   * containing the name/value pairs of that sharing profile's parameters.
   *
   * @param sharingProfile The sharing profile whose configuration should be used to produce the
   *                       collection of parameter models.
   * @return A collection of parameter models containing the name/value pairs of the given sharing
  ...
  /**
   * Retrieves all parameters visible to the given user and associated with the sharing profile
   * having the given identifier. If the given user has no access to such parameters, or no such
   * sharing profile exists, the returned map will be empty.
   *
   * @param user       The user retrieving sharing profile parameters.
   * @param identifier The identifier of the sharing profile whose parameters are being retrieved.
