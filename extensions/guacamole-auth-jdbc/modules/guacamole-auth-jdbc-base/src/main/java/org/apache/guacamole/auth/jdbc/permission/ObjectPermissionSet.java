 ...
 * manipulate the object-level permissions associated with a particular entity.
 */
public abstract class ObjectPermissionSet extends RestrictedObject
  ...
  /**
   * The entity associated with this permission set. Each of the permissions in this permission set
   * is granted to this entity.
   */
  private ModeledPermissions<? extends EntityModel> entity;

  /**
   * The identifiers of all groups that should be taken into account when determining the
   * permissions effectively granted to the entity.
   */
  private Set<String> effectiveGroups;

  /**
   * Creates a new ObjectPermissionSet. The resulting permission set must still be initialized by a
   * call to init(), or the information necessary to read and modify this set will be missing.
   */
  public ObjectPermissionSet() {
  }

  /**
   * Initializes this permission set with the current user and the entity to whom the permissions in
   * this set are granted.
   *
   * @param currentUser     The user who queried this permission set, and whose permissions dictate
   *                        the access level of all operations performed on this set.
   * @param entity          The entity to whom the permissions in this set are granted.
  ...
  /**
   * Returns an ObjectPermissionService implementation for manipulating the type of permissions
   * contained within this permission set.
   *
   * @return An object permission service for manipulating the type of permissions contained within
   * this permission set.
   */
  ...
  public Set<ObjectPermission> getPermissions() throws GuacamoleException {
    return getObjectPermissionService().retrievePermissions(getCurrentUser(), entity,
        effectiveGroups);
  }
  ...
  public boolean hasPermission(ObjectPermission.Type permission,
    ...
    return getObjectPermissionService().hasPermission(getCurrentUser(), entity, permission,
        identifier, effectiveGroups);
  }
  ...
  public Collection<String> getAccessibleObjects(Collection<ObjectPermission.Type> permissions,
    ...
    return getObjectPermissionService().retrieveAccessibleIdentifiers(getCurrentUser(), entity,
