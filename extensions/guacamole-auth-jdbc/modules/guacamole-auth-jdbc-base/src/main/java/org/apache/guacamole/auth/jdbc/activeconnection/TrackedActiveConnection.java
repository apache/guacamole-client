...
/**
 * An implementation of the ActiveConnection object which has an associated ActiveConnectionRecord.
 */
public class TrackedActiveConnection extends RestrictedObject implements ActiveConnection {
  ...
  /**
   * The actual connection record from which this ActiveConnection derives its data.
   */
  ...
  /**
   * Initializes this TrackedActiveConnection, copying the data associated with the given active
   * connection record. At a minimum, the identifier of this active connection will be set, the
   * start date, and the identifier of the associated connection will be copied. If requested,
   * sensitive information like the associated username will be copied, as well.
   *
   * @param currentUser                 The user that created or retrieved this object.
   * @param activeConnectionRecord      The active connection record to copy.
   * @param includeSensitiveInformation Whether sensitive data should be copied from the connection
   *                                    record as well. This includes the remote host, associated
   *                                    tunnel, and username.
   * @param connectable                 Whether the user that retrieved this object should be
   *                                    allowed to join the active connection.
   */
  ...
  /**
   * Returns the connection being actively used. If this active connection is not the primary
   * connection, this will be the connection being actively shared.
   *
   * @return The connection being actively used.
   */
  ...
  /**
   * Shares this active connection with the user that retrieved it, returning a
   * SharedConnectionDefinition that can be used to establish a tunnel to the shared connection. If
   * provided, access within the shared connection will be restricted by the sharing profile with
   * the given identifier.
   *
   * @param identifier The identifier of the sharing profile that defines the restrictions applying
   *                   to the shared connection, or null if no such restrictions apply.
   * @return A new SharedConnectionDefinition which can be used to establish a tunnel to the shared
   * connection.
   * @throws GuacamoleException If permission to share this active connection is denied.
   */
