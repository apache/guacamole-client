...
/**
 * Service which provides convenience methods for creating, retrieving, and manipulating
 * connections.
 */
public class ConnectionService extends
    ModeledChildDirectoryObjectService<ModeledConnection, Connection, ConnectionModel> {
  ...
  protected void beforeCreate(ModeledAuthenticatedUser user,
    ...
    ConnectionModel existing = connectionMapper.selectOneByName(model.getParentIdentifier(),
        model.getName());
    if (existing != null) {
      throw new GuacamoleClientException(
          "The connection \"" + model.getName() + "\" already exists.");
    }
  ...
  /**
   * Given an arbitrary Guacamole connection, produces a collection of parameter model objects
   * containing the name/value pairs of that connection's parameters.
   *
   * @param connection The connection whose configuration should be used to produce the collection
   *                   of parameter models.
   * @return A collection of parameter models containing the name/value pairs of the given
  ...
  /**
   * Returns the set of all identifiers for all connections within the connection group having the
   * given identifier. Only connections that the user has read access to will be returned.
   * <p>
   * Permission to read the connection group having the given identifier is NOT checked.
   *
   * @param user       The user retrieving the identifiers.
   * @param identifier The identifier of the parent connection group, or null to check the root
   *                   connection group.
   * @return The set of all identifiers for all connections in the connection group having the given
   * identifier that the user has read access to.
   * @throws GuacamoleException If an error occurs while reading identifiers.
   */
  ...
  /**
   * Retrieves all parameters visible to the given user and associated with the connection having
   * the given identifier. If the given user has no access to such parameters, or no such connection
   * exists, the returned map will be empty.
   *
   * @param user       The user retrieving connection parameters.
   * @param identifier The identifier of the connection whose parameters are being retrieved.
   * @return A new map of all parameter name/value pairs that the given user has access to.
   */
   ...
   *
   * @param models The model objects to use to back the connection record objects within the
   *               returned list.
   * @return A list of connection record objects which are backed by the models in the given list.
   */
  ...
  /**
   * Retrieves the connection history records matching the given criteria. Retrieves up to
   * <code>limit</code> connection history records matching the given terms and sorted by the given
   * predicates. Only history records associated with data that the given user can read are
   * returned.
   *
   * @param identifier       The optional connection identifier for which history records should be
   *                         retrieved, or null if all readable records should be retrieved.
   * @param user             The user retrieving the connection history.
   ...
   * @return The connection history of the given connection, including any active connections.
   * @throws GuacamoleException If permission to read the connection history is denied.
   */
  ...
  /**
   * Connects to the given connection as the given user, using the given client information. If the
   * user does not have permission to read the connection, permission will be denied.
   *
   * @param user       The user connecting to the connection.
   * @param connection The connection being connected to.
   * @param info       Information associated with the connecting client.
   * @param tokens     A Map containing the token names and corresponding values to be applied as
   *                   parameter tokens when establishing the connection.
   * @return A connected GuacamoleTunnel associated with a newly-established connection.
   * @throws GuacamoleException If permission to connect to this connection is denied.
   */
