...
/**
 * Service which verifies compliance with the password policy configured via guacamole.properties.
 */
public class PasswordPolicyService {

  /**
   * Regular expression which matches only if the string contains at least one lowercase character.
   */
  private final Pattern CONTAINS_LOWERCASE = Pattern.compile("\\p{javaLowerCase}");
  /**
   * Regular expression which matches only if the string contains at least one uppercase character.
   */
  private final Pattern CONTAINS_UPPERCASE = Pattern.compile("\\p{javaUpperCase}");
  /**
   * Regular expression which matches only if the string contains at least one numeric character.
   */
  private final Pattern CONTAINS_DIGIT = Pattern.compile("\\p{Digit}");
  /**
   * Regular expression which matches only if the string contains at least one non-alphanumeric
   * character.
   */
   ...
   *
   * @param str      The string to test against all provided regular expressions.
   * @param patterns The regular expressions to match against the given string.
   * @return true if the given string matches all provided regular expressions, false otherwise.
   */
  ...
  /**
   * Returns whether the given password matches any of the user's previous passwords. Regardless of
   * the value specified here, the maximum number of passwords involved in this check depends on how
   * many previous passwords were actually recorded, which depends on the password policy.
   *
   * @param password    The password to check.
   * @param username    The username of the user whose history should be compared against the given
   *                    password.
   * @param historySize The maximum number of history records to compare the password against.
   * @return true if the given password matches any of the user's previous passwords, up to the
   * specified limit, false otherwise.
   */
  ...
  /**
   * Verifies that the given new password complies with the password policy configured within
   * guacamole.properties, throwing a GuacamoleException if the policy is violated in any way.
   *
   * @param username The username of the user whose password is being changed.
   * @param password The proposed new password.
   * @throws GuacamoleException If the password policy cannot be parsed, or if the proposed password
  ...
  public void verifyPassword(String username, String password)
    ...
    if (policy.isMultipleCaseRequired() && !matches(password, CONTAINS_LOWERCASE,
        CONTAINS_UPPERCASE)) {
  ...
  /**
   * Returns the age of the given user's password, in days. The age of a user's password is the
   * amount of time elapsed since the password was last changed or reset.
   *
   * @param user The user to calculate the password age of.
  ...
  /**
   * Verifies that the given user can change their password without violating password aging policy.
   * If changing the user's password would violate the aging policy, a GuacamoleException will be
   * thrown.
   *
   * @param user The user whose password is changing.
   * @throws GuacamoleException If the user's password cannot be changed due to the password aging
   ...
   * @return true if the user needs to change their password to comply with the password aging
   * policy, false otherwise.
   * @throws GuacamoleException If the password policy cannot be parsed.
   */
  ...
  /**
   * Records the password that was associated with the given user at the time the user was queried,
   * such that future attempts to set that same password for that user will be denied. The number of
   * passwords remembered for each user is limited by the password policy.
   *
   * @param user The user whose password should be recorded within the password history.
   * @throws GuacamoleException If the password policy cannot be parsed.
   */
