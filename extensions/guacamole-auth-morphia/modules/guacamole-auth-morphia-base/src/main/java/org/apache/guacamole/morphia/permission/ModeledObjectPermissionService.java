 ...
 */
public abstract class ModeledObjectPermissionService extends
        ModeledPermissionService<ObjectPermissionSet, ObjectPermission, ObjectPermissionModel>
     ...
     *
     * @param permissions
     *            The permissions that are being added or removed from the
     *            target user.
     *
     * @return true if the user has permission to change the target users
    ...
    protected boolean canAlterPermissions(ModeledAuthenticatedUser user,
        ...
        ObjectPermissionSet userPermissionSet = user.getUser()
                .getUserPermissions();
    ...
    public ObjectPermission retrievePermission(ModeledAuthenticatedUser user,
        ...
        if (canReadPermissions(user, targetUser)) {
            ...
            ObjectPermissionModel model = getPermissionMapper()
                    .selectOne(targetUser.getModel(), type, identifier);
            if (model == null)
                return null;
