 ...
 */
public class SharingProfileService extends
        ModeledChildDirectoryObjectService<ModeledSharingProfile, SharingProfile, SharingProfileModel> {
    ...
    protected ModeledSharingProfile getObjectInstance(
            ModeledAuthenticatedUser currentUser, SharingProfileModel model) {
    ...
    protected void beforeCreate(ModeledAuthenticatedUser user,
        ...
        SharingProfileModel existing = sharingProfileMapper
     ...
     */
    private Collection<SharingProfileParameterModel> getParameterModels(
            ModeledSharingProfile sharingProfile) {

        Map<String, String> parameters = sharingProfile.getParameters();

        // Convert parameters to model objects
        Collection<SharingProfileParameterModel> parameterModels = new ArrayList<SharingProfileParameterModel>(
                parameters.size());
        for (Map.Entry<String, String> parameterEntry : parameters.entrySet()) {
            ...
            listIdentifiersSharingProfile.add(sharingProfile.getIdentifier());
            List<SharingProfileModel> listConnections = (List<SharingProfileModel>) sharingProfileMapper
            ...
            model.setSharingProfile(listConnections.get(0));
            model.setName(name);
    ...
    public ModeledSharingProfile createObject(ModeledAuthenticatedUser user,
        ...
        Collection<SharingProfileParameterModel> parameterModels = getParameterModels(
                sharingProfile);
    ...
    public Map<String, String> retrieveParameters(ModeledAuthenticatedUser user,
        ...
        if (canRetrieveParameters) {
            for (SharingProfileParameterModel parameter : parameterMapper
                    .select(identifier))
    ...
    protected Class<? extends ObjectPermissionModel> getClassPermissionModel() {
        return SharingProfilePermissionModel.class;
    }
