...
package org.apache.guacamole.morphia.tunnel;
...
import org.apache.guacamole.morphia.sharing.connection.SharedConnectionDefinition;
...
import org.apache.guacamole.morphia.user.RemoteAuthenticatedUser;
...
public abstract class AbstractGuacamoleTunnelService
    ...
    private final Logger logger = LoggerFactory
            .getLogger(AbstractGuacamoleTunnelService.class);
    ...
    /**
     * Acquires possibly-exclusive access to any one of the given connections on
     * behalf of the given user. If access is denied for any reason, or if no
     * connection is available, an exception is thrown.
     *
     * @param user
     ...
     *            Whether connections which have been designated for use in
     *            failover situations only (hot spares) may be considered.
     *
     * @return The connection that has been acquired on behalf of the given
     *         user.
     *
     ...
     *
     * @param connection
     *            The connection whose protocol and parameters should be added
     *            to the returned configuration.
     *
     * @return A GuacamoleConfiguration containing the protocol and parameters
    ...
    private GuacamoleConfiguration getGuacamoleConfiguration(
        ...
        Collection<ConnectionParameterModel> parameters = connectionParameterMapper
                .select(connection.getIdentifier());
     ...
     *            The sharing profile whose associated parameters dictate the
     *            level of access granted to the user joining the connection.
     *
     * @param connectionID
     *            The ID of the connection being joined, as provided by guacd
     *            when the original connection was established, or null if a new
     *            connection should be created instead.
     *
     * @return A GuacamoleConfiguration containing the protocol and parameters
     *         from the given connection.
     */
    private GuacamoleConfiguration getGuacamoleConfiguration(
            RemoteAuthenticatedUser user, ModeledSharingProfile sharingProfile,
            String connectionID) {
        ...
        // Set parameters from associated data
        Collection<SharingProfileParameterModel> parameters = sharingProfileParameterMapper
    ...
    private void saveConnectionRecord(ActiveConnectionRecord record) {
        ...
        List<String> listIdentifiersConnection = new ArrayList<String>();
        listIdentifiersConnection.add(record.getConnectionIdentifier());
        List<ConnectionModel> listConnections = (List<ConnectionModel>) connectionMapper
                .select(listIdentifiersConnection);

        List<String> listIdentifiersSharingProfile = new ArrayList<String>();
        listIdentifiersSharingProfile.add(record.getSharingProfileIdentifier());
        List<SharingProfileModel> listSharingProfile = (List<SharingProfileModel>) sharingProfileMapper
                .select(listIdentifiersSharingProfile);

        // Copy user information and timestamps into new record
        recordModel.setUsername(record.getUsername());
        if (!listConnections.isEmpty()) {
            recordModel.setConnection(listConnections.get(0));
        }
        recordModel.setConnectionName(record.getConnectionName());
        recordModel.setRemoteHost(record.getRemoteHost());
        if (!listSharingProfile.isEmpty()) {
            recordModel.setSharingProfile(listSharingProfile.get(0));
        }
     ...
     *            The callback which should be invoked whenever the returned
     *            socket closes.
     *
     * @return An unconfigured GuacamoleSocket, already connected to guacd.
     *
     * @throws GuacamoleException
     *             If an error occurs while connecting to guacd, or while
     *             parsing guacd-related properties.
     */
    private GuacamoleSocket getUnconfiguredGuacamoleSocket(
        ...
        case SSL:
            return new ManagedSSLGuacamoleSocket(proxyConfig.getHostname(),
                    proxyConfig.getPort(), socketClosedCallback);

        // Use straight TCP if unencrypted
        case NONE:
            return new ManagedInetGuacamoleSocket(proxyConfig.getHostname(),
                    proxyConfig.getPort(), socketClosedCallback);
    ...
    private class ConnectionCleanupTask implements Runnable {
        ...
        /**
         * Creates a new task which automatically cleans up after the connection
         * associated with the given ActiveConnectionRecord. The connection and
         * parent group will be removed from the maps of active connections and
         * groups, and exclusive access will be released.
         *
         * @param activeConnection
         *            The ActiveConnectionRecord whose associated connection
         *            should be cleaned up once this task runs.
         */
        ...
        public void run() {
            ...
            // Release the associated connection if this is the primary
            // connection
            if (activeConnection.isPrimaryConnection()) {
                ...
                activeConnectionGroups.remove(parentIdentifier,
                        activeConnection);
     ...
     *
     * @throws GuacamoleException
     *             If an error occurs while the connection is being established,
     *             or while connection configuration information is being
     *             retrieved.
     */
    private GuacamoleTunnel assignGuacamoleTunnel(
            ActiveConnectionRecord activeConnection,
            GuacamoleClientInformation info, boolean interceptErrors)
            throws GuacamoleException {

        // Record new active connection
        Runnable cleanupTask = new ConnectionCleanupTask(activeConnection);
        activeTunnels.put(activeConnection.getUUID().toString(),
                activeConnection);
        ...
        try {
            ...
            // Retrieve connection information associated with given connection
            // record
            ModeledConnection connection = activeConnection.getConnection();
            ...
            // which does so
                ...
                if (connectionID == null)
                    throw new GuacamoleResourceNotFoundException(
                            "No existing connection to be joined.");
            ...
            ConfiguredGuacamoleSocket socket = new ConfiguredGuacamoleSocket(
                    getUnconfiguredGuacamoleSocket(
                            connection.getGuacamoleProxyConfiguration(),
                            cleanupTask),
                    config, info);

            // Assign and return new tunnel
            if (interceptErrors)
                return activeConnection.assignGuacamoleTunnel(
                        new FailoverGuacamoleSocket(socket),
     ...
     *
     * @param user
     *            The user whose preferred connections should be used to filter
     *            the given collection of connection identifiers.
     *
     * @param identifiers
     *            The collection of connection identifiers that should be
     *            filtered.
     *
     * @return A collection of connection identifiers containing only the subset
     *         of connection identifiers which are also preferred or, if none of
     *         the provided identifiers are preferred, the original collection
     *         of identifiers.
     */
    private Collection<String> getPreferredConnections(
     ...
     *
     * @param user
     *            The user on whose behalf the balanced connections within the
     *            given connection group are being retrieved.
     *
     * @param connectionGroup
     *            The connection group to retrieve the balanced connections of.
     *
     * @return A list containing all balanced connections within the given
     *         group, or an empty list if there are no such connections.
     */
    private List<ModeledConnection> getBalancedConnections(
        ...
        Collection<String> identifiers = connectionMapper
                .selectIdentifiersWithin(connectionGroup.getIdentifier());
        ...
        Collection<ConnectionModel> models = connectionMapper
                .select(identifiers);
        List<ModeledConnection> connections = new ArrayList<ModeledConnection>(
                models.size());
    ...
    public Collection<ActiveConnectionRecord> getActiveConnections(
        ...
        // Build set of all connection identifiers associated with active
        // tunnels
        ...
        Collection<ConnectionModel> connections = connectionMapper
                .selectReadable(user.getUser().getModel(), identifiers);
        ...
        Collection<ActiveConnectionRecord> visibleRecords = new ArrayList<ActiveConnectionRecord>(
                records.size());
        for (ActiveConnectionRecord record : records) {
    ...
    @Override
    public GuacamoleTunnel getGuacamoleTunnel(
            final ModeledAuthenticatedUser user,
        ...
        ActiveConnectionRecord connectionRecord = activeConnectionRecordProvider
                .get();
    ...
    public GuacamoleTunnel getGuacamoleTunnel(ModeledAuthenticatedUser user,
        ...
        List<ModeledConnection> connections = getBalancedConnections(user,
                connectionGroup);
        ...
        do {
            ...
            try {
                ...
                GuacamoleTunnel tunnel = assignGuacamoleTunnel(connectionRecord,
                        info, connections.size() > 1);

                // If session affinity is enabled, prefer this connection going
                // forward
                if (connectionGroup.isSessionAffinityEnabled())
            ...
            // connections
                ...
                logger.debug(
                        "Upstream remote desktop reported an error during connection.",
                        e);
