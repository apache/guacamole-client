...
import org.apache.guacamole.morphia.base.ModeledChildDirectoryObjectService;
import org.apache.guacamole.morphia.base.ModeledDirectoryObjectMapper;
import org.apache.guacamole.morphia.permission.ConnectionPermissionMapper;
import org.apache.guacamole.morphia.permission.ObjectPermissionMapper;
...
public class ConnectionService extends
    ...
    protected ModeledConnection getObjectInstance(
            ModeledAuthenticatedUser currentUser, ConnectionModel model) {
    ...
    @Override
    protected ConnectionModel getModelInstance(
    ...
    @Override
    protected ObjectPermissionSet getPermissionSet(
    ...
    @Override
    protected ObjectPermissionSet getParentPermissionSet(
            ModeledAuthenticatedUser user) throws GuacamoleException {
    ...
    protected void beforeCreate(ModeledAuthenticatedUser user,
        ...
        if (model.getName() == null || model.getName().trim().isEmpty())
            throw new GuacamoleClientException(
                    "Connection names must not be blank.");

        // Do not attempt to create duplicate connections
        ConnectionModel existing = connectionMapper
                .selectOneByName(model.getParent(), model.getName());
        if (existing != null)
            throw new GuacamoleClientException("The connection \""
     ...
     */
    private Collection<ConnectionParameterModel> getParameterModels(
        ...
        Collection<ConnectionParameterModel> parameterModels = new ArrayList<ConnectionParameterModel>(
                parameters.size());
        for (Map.Entry<String, String> parameterEntry : parameters.entrySet()) {
            ...
            listIdentifiersConnection.add(connection.getIdentifier());
            ...
            model.setConnection(listConnections.get(0));
            model.setName(name);
    ...
    @Override
    public ModeledConnection createObject(ModeledAuthenticatedUser user,
        ...
        Collection<ConnectionParameterModel> parameterModels = getParameterModels(
                connection);
    ...
    public Map<String, String> retrieveParameters(ModeledAuthenticatedUser user,
        ...
        if (canRetrieveParameters) {
            for (ConnectionParameterModel parameter : parameterMapper
                    .select(identifier))
     ...
     *
     * @return A list of connection record objects which are backed by the
     *         models in the given list.
     */
    protected List<ConnectionRecord> getObjectInstances(
            List<ConnectionRecordModel> models) {

        // Create new list of records by manually converting each model
        List<ConnectionRecord> objects = new ArrayList<ConnectionRecord>(
                models.size());
    ...
    public List<ConnectionRecord> retrieveHistory(ModeledAuthenticatedUser user,
        ...
        if (hasObjectPermission(user, identifier, ObjectPermission.Type.READ)) {
            ...
            // Retrieve history
            List<ConnectionRecordModel> models = connectionRecordMapper
                    .select(identifier);

            // Get currently-active connections
            List<ConnectionRecord> records = new ArrayList<ConnectionRecord>(
                    tunnelService.getActiveConnections(connection));
    ...
    @Override
    protected Class<? extends ObjectPermissionModel> getClassPermissionModel() {
        return ConnectionPermissionModel.class;
    }
