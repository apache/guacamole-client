...
public abstract class ModeledChildDirectoryObjectService<InternalType extends ModeledChildDirectoryObject<ModelType>, ExternalType extends Identifiable, ModelType extends ChildObjectModel>
        extends
        ModeledDirectoryObjectService<InternalType, ExternalType, ModelType> {
     ...
     *
     * @param identifier
     *            The identifier of the object that has been modified, if it
     *            exists. If the object is being created, this will be null.
     *
     * @param model
     *            The model that has been modified, if any. If the object is
     *            being deleted, this will be null.
     *
     * @return A collection of the identifiers of all parents that will be
     *         affected (updated) by the change.
     *
     * @throws GuacamoleException
     *             If an error occurs while determining which parents are
     *             affected.
     */
    protected Collection<String> getModifiedParents(
            ModeledAuthenticatedUser user, String identifier, ModelType model)
            throws GuacamoleException {
        ...
        if (identifier != null) {
            ...
            oldParentIdentifier = current.getParent();
        }
        ...
        if (model != null) {
            ...
            parentIdentifier = model.getParent();

            // If both parents have the same identifier, nothing has changed
            if (parentIdentifier != null
                    && parentIdentifier.equals(oldParentIdentifier))
     ...
     *            being deleted, this will be null.
     *
     * @return true if the user has update permission for all modified parents,
     *         false otherwise.
     *
     * @throws GuacamoleException
     *             If an error occurs while determining which parents are
    ...
    protected boolean canUpdateModifiedParents(ModeledAuthenticatedUser user,
        ...
        Collection<String> modifiedParents = getModifiedParents(user,
                identifier, model);
