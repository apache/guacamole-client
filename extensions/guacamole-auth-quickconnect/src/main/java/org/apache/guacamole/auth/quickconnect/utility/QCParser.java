...
/**
 * A utility class to parse out a URI into the settings necessary to generate a
...
public class QCParser {
  ...
  /**
   * The list of parameters that are allowed to be placed into a configuration by this parser. If
   * not defined, all parameters will be allowed unless explicitly denied.
   */
  private final List<String> allowedParams;

  /**
   * The list of parameters that are explicitly denied from being placed into a configuration by
   * this parser.
   */
  private final List<String> deniedParams;

  /**
   * Create a new instance of the QCParser class, with the provided allowed and denied parameter
   * lists, if any.
   *
   * @param allowedParams A list of parameters that are allowed to be parsed and placed into a
   *                      connection configuration, or null or empty if all parameters are allowed.
   * @param deniedParams  A list of parameters, if any, that should be explicitly denied from being
   *                      placed into a connection configuration.
   */
  ...
  /**
   * Create a new instance of the QCParser class, initializing the allowed and denied parameter
   * lists to empty lists, which means all parameters will be allowed and none will be denied.
   */
  public QCParser() {
  ...
  /**
   * Parse out a URI string and get a GuacamoleConfiguration from that string, or an exception if
   * the parsing fails.
   *
   * @param uri The string form of the URI to be parsed.
   * @return A GuacamoleConfiguration generated using the information provided by the user in the
   * URI.
   * @throws GuacamoleException If an error occurs parsing the URI.
   */
  ...
  /**
   * Parse the given string for parameter key/value pairs and update the provided
   * GuacamoleConfiguration object with the parsed values, checking to make sure that the parser is
   * allowed to provide the requested parameters.
   *
   * @param queryStr The query string to parse for key/value pairs.
   * @param config   The GuacamoleConfiguration object that should be updated with the parsed
   *                 parameters.
   * @throws GuacamoleException If Java unexpectedly lacks UTF-8 support.
   */
  ...
  /**
   * Parse the given string for username and password values, and, if values are present and allowed
   * by the configuration, decode them and set them in the provided GuacamoleConfiguration object.
   *
   * @param userInfo The string to parse for username/password values.
   * @param config   The GuacamoleConfiguration object to store the username and password in.
   * @throws GuacamoleException If Java unexpectedly lacks UTF-8 support.
   */
  ...
  /**
   * Given a GuacamoleConfiguration object, generate a name for the configuration based on the
   * protocol, host, user and port in the configuration, and return the string value.
   *
   * @param config The GuacamoleConfiguration object to use to generate the name.
   * @return The String value of the name that is generated.
   * @throws GuacamoleException If an error occurs getting items in the configuration.
   */
  ...
  /**
   * For a given parameter, check to make sure the parameter is allowed to be used in the connection
   * configuration, first by checking to see if allowed parameters are defined and the given
   * parameter is present, then by checking for explicitly denied parameters. Returns false if the
   * configuration prevents the parameter from being used, otherwise true.
   *
   * @param param The name of the parameter to check.
   * @return False if the configuration prevents the parameter from being used, otherwise true.
   */
