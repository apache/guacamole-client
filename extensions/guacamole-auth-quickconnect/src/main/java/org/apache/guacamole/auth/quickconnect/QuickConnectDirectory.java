...
/**
 * Implementation of a directory to store Connection objects completely in memory.
 */
public class QuickConnectDirectory extends SimpleDirectory<Connection> {
  ...
  /**
   * Initialize the QuickConnectDirectory with the default empty Map for Connection objects, and the
   * specified ConnectionGroup at the root of the directory.
   *
   * @param rootGroup A group that should be at the root of this directory.
   */
   ...
   *
   * @return An int representing the next available connection identifier to be used when adding
   * connections.
   */
  ...
  /**
   * Taking a URI, parse the URI into a GuacamoleConfiguration object and then use the configuration
   * to create a SimpleConnection object, obtain an identifier, and place it on the tree, returning
   * the identifier value of the new connection.
   *
   * @param uri The URI to parse into a GuacamoleConfiguration, which will then be used to generate
