...
/**
 * Service which provides a centralized means of establishing connections, tracking/joining active
 * connections, and retrieving associated data.
 */
@Singleton
public class ConnectionService {
  ...
  /**
   * Mapping of the unique IDs of active connections (as specified within the UserData.Connection
   * object) to the underlying connection ID (as returned via the Guacamole protocol handshake).
   * Only connections with defined IDs are tracked here.
   */
  private final ConcurrentHashMap<String, String> activeConnections =
      new ConcurrentHashMap<>();
  /**
   * Mapping of the connection IDs of joinable connections (as returned via the Guacamole protocol
   * handshake) to the Collection of tunnels shadowing those connections.
   */
  ...
  /**
   * Generates a new GuacamoleConfiguration from the associated protocol and parameters of the given
   * UserData.Connection. If the configuration cannot be generated (because a connection is being
   * joined by that connection is not actually active), null is returned.
   *
   * @param connection The UserData.Connection whose protocol and parameters should be used to
   *                   construct the new GuacamoleConfiguration.
   * @return A new GuacamoleConfiguration generated from the associated protocol and parameters of
   * the given UserData.Connection, or null if the configuration cannot be generated.
   */
  ...
  /**
   * Closes all tunnels within the given connection. If a GuacamoleException is thrown by any tunnel
   * during closure, that exception is ignored.
   *
   * @param tunnels The Collection of tunnels to close.
   */
  ...
  /**
   * Establishes a connection to guacd using the information associated with the given connection
   * object. The resulting connection will be provided the given client information during the
   * Guacamole protocol handshake.
   *
   * @param connection The connection object describing the nature of the connection to be
   *                   established.
   * @param info       Information associated with the connecting client.
   * @return A fully-established GuacamoleTunnel.
   * @throws GuacamoleException If an error occurs while connecting to guacd, or if permission to
   *                            connect is denied.
   */
