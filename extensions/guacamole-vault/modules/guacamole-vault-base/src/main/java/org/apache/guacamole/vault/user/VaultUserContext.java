...
/**
 * UserContext implementation which automatically injects tokens containing the values of secrets
 * retrieved from a vault.
 */
public class VaultUserContext extends TokenInjectingUserContext {
  ...
  /**
   * The name of the token which will be replaced with the username of the current user if specified
   * within the name of a secret. Unlike the standard GUAC_USERNAME token, the username stored with
   * the object representing the user is used here, not necessarily the username provided during
   * authentication. This token applies to both connections and connection groups.
   */
  private static final String USERNAME_TOKEN = "USERNAME";

  /**
   * The name of the token which will be replaced with the name of the current connection group if
   * specified within the name of a secret. This token only applies only to connection groups.
   */
  private static final String CONNECTION_GROUP_NAME_TOKEN = "CONNECTION_GROUP_NAME";

  /**
   * The name of the token which will be replaced with the identifier of the current connection
   * group if specified within the name of a secret. This token only applies only to connection
   * groups.
   */
  private static final String CONNECTION_GROUP_IDENTIFIER_TOKEN = "CONNECTION_GROUP_ID";

  /**
   * The name of the token which will be replaced with the \"hostname\" connection parameter of the
   * current connection if specified within the name of a secret. If the \"hostname\" parameter
   * cannot be retrieved, or if the parameter is blank, the token will not be replaced and any
   * secrets involving that token will not be retrieved. This token only applies only to
   * connections.
   */
  private static final String CONNECTION_HOSTNAME_TOKEN = "CONNECTION_HOSTNAME";

  /**
   * The name of the token which will be replaced with the \"username\" connection parameter of the
   * current connection if specified within the name of a secret. If the \"username\" parameter
   * cannot be retrieved, or if the parameter is blank, the token will not be replaced and any
   * secrets involving that token will not be retrieved. This token only applies only to
   * connections.
   */
  ...
  /**
   * The name of the token which will be replaced with the name of the current connection if
   * specified within the name of a secret. This token only applies only to connections.
   */
  private static final String CONNECTION_NAME_TOKEN = "CONNECTION_NAME";

  /**
   * The name of the token which will be replaced with the identifier of the current connection if
   * specified within the name of a secret. This token only applies only to connections.
   */
  ...
  /**
   * Creates a new VaultUserContext which automatically injects tokens containing values of secrets
   * retrieved from a vault. The given UserContext is decorated such that connections and connection
   * groups will receive additional tokens during the connection process.
   * <p>
   * Note that this class depends on concrete implementations of the following classes to be
   * provided via dependency injection:
   * <p>
   * - VaultConfigurationService - VaultSecretService
   * <p>
   * Bindings providing these concrete implementations will need to be provided by subclasses of
   * VaultAuthenticationProviderModule for each supported vault.
   *
  ...
  /**
   * Creates a new TokenFilter instance with token values set for all tokens which are not specific
   * to connections or connection groups. Currently, this is only the vault-specific username token
   * ("USERNAME"). Each token stored within the returned TokenFilter via setToken() will be
   * automatically canonicalized for use within secret names.
   *
   * @return A new TokenFilter instance with token values set for all tokens which are not specific
   * to connections or connection groups.
   */
  ...
  /**
   * Initiates asynchronous retrieval of all applicable tokens and corresponding values from the
   * vault, using the given TokenFilter to filter tokens within the secret names prior to retrieving
   * those secrets.
   *
   * @param tokenMapping     The mapping dictating the name of the secret which maps to each
   *                         parameter token, where the key is the name of the parameter token and
   *                         the value is the name of the secret. The name of the secret may contain
   *                         its own tokens, which will be substituted using values from the given
   *                         filter.
   * @param secretNameFilter The filter to use to substitute values for tokens in the names of
   *                         secrets to be retrieved from the vault.
   * @param config           The GuacamoleConfiguration of the connection for which tokens are being
   *                         retrieved, if available. This may be null.
   * @param configFilter     A TokenFilter instance that applies any tokens already available to be
   *                         applied to the configuration of the Guacamole connection. These tokens
   *                         will consist of tokens already supplied to connect().
   * @return A Map of token name to Future, where each Future represents the pending retrieval
   * operation which will ultimately be completed with the value of all secrets mapped to that
   * token.
   * @throws GuacamoleException If the value for any applicable secret cannot be retrieved from the
  ...
  /**
   * Waits for all pending secret retrieval operations to complete, transforming each Future within
   * the given Map into its contained String value.
   *
   * @param pendingTokens A Map of token name to Future, where each Future represents the pending
   *                      retrieval operation which will ultimately be completed with the value of
   *                      all secrets mapped to that token.
   * @return A Map of token name to the corresponding String value retrieved for that token from the
   * vault.
   * @throws GuacamoleException If the value for any applicable secret cannot be retrieved from the
   *                            vault due to an error.
   */
  ...
  /**
   * Retrieves the GuacamoleConfiguration of the given Connection. If possible, privileged access to
   * the configuration is obtained first. Note that the underlying extension is not required to
   * allow privileged access, nor is it required to expose the underlying configuration at all.
   *
   * @param connection The connection to retrieve the configuration from.
   * @return The GuacamoleConfiguration associated with the given connection, which may be partial
   * or empty.
   * @throws GuacamoleException If an error prevents privileged retrieval of the configuration.
   */
