...
/**
 * ActivityRecordSet implementation that automatically defines ActivityLogs for files that relate to
 * history entries within the wrapped set.
 */
public class RecordedConnectionActivityRecordSet extends
    DecoratingActivityRecordSet<ConnectionRecord> {
  ...
  /**
   * Creates a new RecordedConnectionActivityRecordSet that wraps the given ActivityRecordSet,
   * automatically associating history entries with ActivityLogs based on related files (session
   * recordings, typescripts, etc.).
   *
   * @param currentUser       The current Guacamole user.
   * @param activityRecordSet The ActivityRecordSet to wrap.
   * @throws GuacamoleException If the permissions for the current user cannot be retrieved.
   */
  ...
  /**
   * Returns whether the current user has permission to view the logs associated with the given
   * history record. It is already given that the user has permission to view the history record
   * itself. This extension considers a user to have permission to view history logs if they are an
   * administrator or if they have permission to edit the associated connection.
   *
   * @param record The record to check.
   * @return true if the current user has permission to view the logs associated with the given
