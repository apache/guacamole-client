...
export class PlayerHeatmapService {
     ...
     */
    private createPath(bucketizedData: number[], maxBucketValue: number, height: number, width: number): string {
        ...
        const scaleYValue = (yValue: number) => height - (yValue * yScalingFactor);
     ...
     */
    generateHeatmapPath = (timestamps: number[], duration: number, maxRate: number, height: number, width: number): string => {
        ...
        const smoothed = this.smooth(buckets);
