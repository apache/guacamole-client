...
angular.module('form').factory('ColorScheme', [function defineColorScheme() {
  ...
  var toHexColor = function toHexColor(color) {
    ...
    var groups = /^rgb:([0-9A-Fa-f]{1,4})\/([0-9A-Fa-f]{1,4})\/([0-9A-Fa-f]{1,4})$/.exec(
        color);
    if (!groups) {
    ...
    return '#' + toHexComponent(groups[1]) + toHexComponent(groups[2])
  ...
  ColorScheme.fromString = function fromString(str) {
    ...
    for (var i = 0; i < statements.length; i++) {
      ...
      var groups = /^\s*(background|foreground|color([0-9]+))\s*:\s*(\S*)\s*$/.exec(
          statement);
      if (!groups) {
        continue;
      }
  ...
  ColorScheme.toString = function toString(scheme) {
    ...
    if (!_.isUndefined(scheme._originalString) && ColorScheme.equals(scheme,
