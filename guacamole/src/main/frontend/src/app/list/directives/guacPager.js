    ...
    controller: ['$scope', function guacPagerController($scope) {
      ...
      var updatePageNumbers = function updatePageNumbers() {
        ...
        if (windowStart < $scope.firstPage) {
          windowEnd = Math.min($scope.lastPage,
        ...
        $scope.pageNumbers = [];
        for (var pageNumber = windowStart; pageNumber <= windowEnd;
            pageNumber++) {
      ...
      var updatePages = function updatePages() {
        ...
        var adjustedCurrentPage = Math.min($scope.lastPage,
      ...
      $scope.selectPage = function selectPage(page) {
        ...
        $scope.previousPage = Math.max($scope.firstPage,
            $scope.currentPage - 1);
