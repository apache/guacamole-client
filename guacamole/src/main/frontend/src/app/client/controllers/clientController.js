    ...
    ['$scope', '$routeParams', '$injector',
      function clientController($scope, $routeParams, $injector) {
        ...
        $scope.applyParameterChanges = function applyParameterChanges(client) {
          angular.forEach($scope.menu.connectionParameters,
        ...
        $scope.connectionListContext = {
          ...
          updateAttachedClients: function updateAttachedClients(id) {
            $scope.addRemoveClient(id,
                !$scope.connectionListContext.attachedClients[id]);
          }
        ...
        const reparseRoute = function reparseRoute() {
          ...
          const previousClients = $scope.clientGroup
              ? $scope.clientGroup.clients.slice() : [];

          // Replace existing group with new group
          setAttachedGroup(
              guacClientManager.getManagedClientGroup($routeParams.id));
          ...
          if (_.isEmpty(
              _.intersection(previousClients, $scope.clientGroup.clients))) {
            $scope.menu.shown = false;
          }
        ...
        .then(function rootGroupsRetrieved(rootConnectionGroups) {
          ...
          var clientPages = userPageService.getClientPages(
              rootConnectionGroups);
        ...
        const isMenuShortcutPressed = function isMenuShortcutPressed(keyboard) {
          ...
          if (_.findKey(keyboard.pressed,
              (val, keysym) => !MENU_KEYS[keysym])) {
            return false;
          }
        ...
        $scope.menuDrag = function menuDrag(inProgress, startX, startY,
        ...
        $scope.$on('guacClientFocused',
            function focusedClientChanged(event, newFocusedClient) {
        ...
        $scope.$watch('focusedClient.thumbnail.canvas',
            function thumbnailChanged(canvas) {
              iconService.setIcons(canvas);
            });

        // Pull sharing profiles once the tunnel UUID is known
        $scope.$watch('focusedClient.tunnel.uuid',
        ...
        $scope.isShared = function isShared() {
          return !!$scope.focusedClient && ManagedClient.isShared(
              $scope.focusedClient);
        };
        ...
        $scope.$on('guacBeforeKeydown',
            function incomingKeydown(event, keysym, keyboard) {
        ...
        $scope.$on('guacBeforeKeyup',
            function incomingKeyup(event, keysym, keyboard) {
              if ($scope.menu.shown) {
                event.preventDefault();
              }
            });
        ...
        $scope.$on('guacKeydown',
            function keydownListener(event, keysym, keyboard) {
              ...
              if (END_KEYS[keysym]
                  && _.findKey(ALT_KEYS,
                      (val, keysym) => keyboard.pressed[keysym])
                  && _.findKey(CTRL_KEYS,
                      (val, keysym) => keyboard.pressed[keysym])
              ) {
        ...
        $scope.$on('guacKeyup',
            function keyupListener(event, keysym, keyboard) {
              ...
              if (substituteKeysPressed[keysym]) {
                event.preventDefault();
                $scope.$broadcast('guacSyntheticKeyup',
        ...
        $scope.$watch('getTitle(clientGroup)',
            function clientTitleChanged(title) {
        ...
        $scope.isConnectionUnstable = function isConnectionUnstable() {
          return _.findIndex($scope.clientGroup.clients,
              client => client.clientState.tunnelUnstable) !== -1;
        };
        ...
        $scope.uploadFiles = function uploadFiles(files) {
          ...
          for (var i = 0; i < files.length; i++) {
            ManagedClient.uploadFile($scope.filesystemMenuContents.client,
                files[i], $scope.filesystemMenuContents);
          }
