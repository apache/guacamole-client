 ...
 */
angular.module('client').factory('ManagedClientGroup',
    ['$injector', function defineManagedClientGroup($injector) {
      ...
      ManagedClientGroup.getRows = function getRows(group) {
        ...
        return Math.ceil(
            group.clients.length / ManagedClientGroup.getColumns(group));
      ...
      ManagedClientGroup.getName = function getName(group) {
        ...
        // focused clients
        let relevantClients = _.filter(group.clients,
            client => client.clientProperties.focused);
        if (!relevantClients.length) {
          relevantClients = group.clients;
        }

        return _.filter(relevantClients, (client => !!client.name)).map(
            client => client.name).join(', ') || '...';
      ...
      ManagedClientGroup.verifyFocus = function verifyFocus(group) {
        ...
        if (group.clients.length >= 1 && _.findIndex(group.clients,
            client => client.clientProperties.focused) === -1) {
