    ...
    ['$rootScope', '$injector',
      function defineManagedFilesystem($rootScope, $injector) {
        ...
        ManagedFilesystem.refresh = function updateDirectory(filesystem, file) {
          ...
          filesystem.object.requestInputStream(file.streamName,
              function handleStream(stream, mimetype) {
                ...
                if (mimetype !== Guacamole.Object.STREAM_INDEX_MIMETYPE) {
                  stream.sendAck('Unexpected mimetype',
                ...
                reader.onend = function jsonReady() {
                  $rootScope.$evalAsync(function updateFileContents() {
                    ...
                    for (var name in mimetypes) {
                      ...
                      if (name.substring(0, expectedPrefix.length)
                          !== expectedPrefix) {
                        continue;
                      }
         ...
         */
        ManagedFilesystem.getInstance = function getInstance(client, object,
            name) {
         ...
         */
        ManagedFilesystem.downloadFile = function downloadFile(managedFilesystem,
            path) {

          // Request download
          managedFilesystem.object.requestInputStream(path,
              function downloadStreamReceived(stream, mimetype) {
                ...
                tunnelService.downloadStream(
                    managedFilesystem.client.tunnel.uuid, stream, mimetype,
                    filename);
         ...
         */
        ManagedFilesystem.changeDirectory = function changeDirectory(filesystem,
            file) {
