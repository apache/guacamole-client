    ...
    function guacClientController($scope, $injector, $element) {
      ...
      const updateDisplayScale = function updateDisplayScale() {
        ...
        $scope.client.clientProperties.maxScale = Math.max(
            $scope.client.clientProperties.minScale, 3);

        // Clamp zoom level, maintain auto-fit
        if (display.getScale() < $scope.client.clientProperties.minScale
            || $scope.client.clientProperties.autoFit) {
          $scope.client.clientProperties.scale = $scope.client.clientProperties.minScale;
        } else if (display.getScale()
      ...
      const scrollToMouse = function scrollToMouse(mouseState) {
        ...
        const mouse_view_x = mouseState.x + displayContainer.offsetLeft
            - main.scrollLeft;
        const mouse_view_y = mouseState.y + displayContainer.offsetTop
            - main.scrollTop;
      ...
      $scope.$watch('client.clientProperties.scrollLeft',
      ...
      $scope.$watch('client.clientProperties.scrollTop',
      ...
      $scope.$watchGroup([
        ...
        touch.offEach(['touchstart', 'touchmove', 'touchend'],
            handleTouchEvent);
        touchScreen.offEach(['mousedown', 'mousemove', 'mouseup'],
            handleEmulatedMouseEvent);
        touchPad.offEach(['mousedown', 'mousemove', 'mouseup'],
            handleEmulatedMouseEvent);

        // Directly forward local touch events
        if ($scope.client.multiTouchSupport) {
          touch.onEach(['touchstart', 'touchmove', 'touchend'],
              handleTouchEvent);
        ...
        // is not preferred
          ...
          touchPad.onEach(['mousedown', 'mousemove', 'mouseup'],
              handleEmulatedMouseEvent);
        }
      ...
      $scope.$watch('client.clientProperties.scale',
      ...
      $scope.$watch('client.clientProperties.autoFit',
      ...
      $scope.mainElementResized = function mainElementResized() {
        ...
        if (client && display && main.offsetWidth && main.offsetHeight) {
          ...
          ManagedClient.connect($scope.client, main.offsetWidth,
              main.offsetHeight);
      ...
      $scope.clientDrag = function clientDrag(inProgress, startX, startY,
      ...
      $scope.clientPinch = function clientPinch(inProgress, startLength,
          currentLength, centerX, centerY) {
        ...
        if (!initialScale) {
          ...
          initialCenterX = (centerX + $scope.client.clientProperties.scrollLeft)
              / initialScale;
          initialCenterY = (centerY + $scope.client.clientProperties.scrollTop)
              / initialScale;
        }
        ...
        currentScale = Math.max(currentScale,
            $scope.client.clientProperties.minScale);
        currentScale = Math.min(currentScale,
            $scope.client.clientProperties.maxScale);
        ...
        $scope.client.clientProperties.scrollLeft = initialCenterX
            * currentScale - centerX;
        $scope.client.clientProperties.scrollTop = initialCenterY * currentScale
            - centerY;
