...
angular.module('manage').controller('manageConnectionController',
    ['$scope', '$injector',
      function manageConnectionController($scope, $injector) {
         ...
         */
        var loadExistingConnection = function loadExistingConnection(dataSource,
            identifier) {
          return $q.all({
            connection: connectionService.getConnection(dataSource, identifier),
            historyEntries: connectionService.getConnectionHistory(dataSource,
                identifier),
            parameters: connectionService.getConnectionParameters(dataSource,
                identifier)
          })
          .then(function connectionDataRetrieved(values) {
            ...
            angular.forEach(values.historyEntries,
        ...
        var loadRequestedConnection = function loadRequestedConnection() {
          ...
          if (identifier) {
            return loadExistingConnection($scope.selectedDataSource,
                identifier);
          }

          // If we are cloning an existing connection, pull its data instead
          if (cloneSourceIdentifier) {
            return loadClonedConnection($scope.selectedDataSource,
        ...
        $q.all({
          ...
          rootGroup: connectionGroupService.getConnectionGroupTree(
              $scope.selectedDataSource, ConnectionGroup.ROOT_IDENTIFIER,
        ...
        $scope.returnToConnectionList = function returnToConnectionList() {
          $location.url(
              '/settings/' + encodeURIComponent($scope.selectedDataSource)
              + '/connections');
        };
        ...
        $scope.cloneConnection = function cloneConnection() {
          $location.path(
              '/manage/' + encodeURIComponent($scope.selectedDataSource)
              + '/connections').search('clone', identifier);
        };
        ...
        $scope.saveConnection = function saveConnection() {
          ...
          // Save the connection
          return connectionService.saveConnection($scope.selectedDataSource,
              $scope.connection);
        ...
        $scope.deleteConnection = function deleteConnection() {
          return connectionService.deleteConnection($scope.selectedDataSource,
              $scope.connection);
        };
