    ...
    ['$scope', '$injector',
      function manageUserGroupController($scope, $injector) {
        ...
        var loadExistingUserGroup = function loadExistingGroup(dataSource,
            identifier) {
          return $q.all({
            userGroups: dataSourceService.apply(userGroupService.getUserGroup,
                dataSources, identifier),
         ...
         */
        var loadClonedUserGroup = function loadClonedUserGroup(dataSource,
            identifier) {
          return $q.all({
            userGroups: dataSourceService.apply(userGroupService.getUserGroup,
        ...
        var loadRequestedUserGroup = function loadRequestedUserGroup() {
          ...
          if (cloneSourceIdentifier) {
            return loadClonedUserGroup($scope.dataSource,
        ...
        $q.all({
          ...
          permissions: dataSourceService.apply(
              permissionService.getEffectivePermissions, dataSources,
              currentUsername),
        ...
        $scope.getUserGroupURL = function getUserGroupURL(dataSource) {
          return '/manage/' + encodeURIComponent(dataSource) + '/userGroups/'
        ...
        $scope.cloneUserGroup = function cloneUserGroup() {
          $location.path('/manage/' + encodeURIComponent($scope.dataSource)
              + '/userGroups').search('clone', identifier);
        };
        ...
        $scope.saveUserGroup = function saveUserGroup() {
          ...
          if ($scope.dataSource in $scope.userGroups) {
            saveUserGroupPromise = userGroupService.saveUserGroup(
                $scope.dataSource, $scope.userGroup);
          } else {
          ...
          return saveUserGroupPromise.then(function savedUserGroup() {
            return $q.all([
              permissionService.patchPermissions($scope.dataSource,
                  $scope.userGroup.identifier, $scope.permissionsAdded,
                  $scope.permissionsRemoved, true),
              membershipService.patchUserGroups($scope.dataSource,
                  $scope.userGroup.identifier, $scope.parentGroupsAdded,
                  $scope.parentGroupsRemoved, true),
              membershipService.patchMemberUserGroups($scope.dataSource,
                  $scope.userGroup.identifier, $scope.memberGroupsAdded,
                  $scope.memberGroupsRemoved),
              membershipService.patchMemberUsers($scope.dataSource,
                  $scope.userGroup.identifier, $scope.memberUsersAdded,
                  $scope.memberUsersRemoved)
            ]);
          });
        ...
        $scope.deleteUserGroup = function deleteUserGroup() {
          return userGroupService.deleteUserGroup($scope.dataSource,
              $scope.userGroup);
        };
