    ...
    ['$scope', '$injector',
      function manageUserController($scope, $injector) {
        ...
        $scope.getUserURL = function getUserURL(dataSource) {
          return '/manage/' + encodeURIComponent(dataSource) + '/users/'
        ...
        $scope.saveUser = function saveUser() {
          ...
          if ($scope.dataSource in $scope.users) {
            saveUserPromise = userService.saveUser($scope.dataSource,
                $scope.user);
          } else {
            saveUserPromise = userService.createUser($scope.dataSource,
                $scope.user);
          }
          ...
          return saveUserPromise.then(function savedUser() {
            ...
            return $q.all([
              permissionService.patchPermissions($scope.dataSource,
                  $scope.user.username, $scope.permissionsAdded,
                  $scope.permissionsRemoved),
              membershipService.patchUserGroups($scope.dataSource,
                  $scope.user.username, $scope.parentGroupsAdded,
                  $scope.parentGroupsRemoved)
            ]);
