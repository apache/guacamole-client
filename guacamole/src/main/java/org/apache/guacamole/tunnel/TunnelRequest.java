...
/**
 * A request object which provides only the functions absolutely required to retrieve and connect to
 * a tunnel.
 */
public abstract class TunnelRequest {
  ...
  /**
   * The name of the parameter containing the identifier of the AuthenticationProvider associated
   * with the UserContext containing the object to which a tunnel is being requested.
   */
   ...
   * Currently, this may be "c" for a Guacamole connection, or "g" for a Guacamole connection
   * group.
   */
  public static final String TYPE_PARAMETER = "GUAC_TYPE";

  /**
   * The name of the parameter containing the unique identifier of the object to which a tunnel is
   * being requested.
   */
  ...
  /**
   * The name of the parameter specifying one supported audio mimetype. This will normally appear
   * multiple times within a single tunnel request - once for each mimetype.
   */
  public static final String AUDIO_PARAMETER = "GUAC_AUDIO";

  /**
   * The name of the parameter specifying one supported video mimetype. This will normally appear
   * multiple times within a single tunnel request - once for each mimetype.
   */
  public static final String VIDEO_PARAMETER = "GUAC_VIDEO";

  /**
   * The name of the parameter specifying one supported image mimetype. This will normally appear
   * multiple times within a single tunnel request - once for each mimetype.
   */
   ...
   * @return The value of the parameter having the given name, or null if no such parameter was
   * specified.
   */
   ...
   *
   * @param name The name of the parameter to return.
   * @return All values of the parameter having the given name , or null if no such parameter was
   * specified.
   */
  public abstract List<String> getParameterValues(String name);

  /**
   * Returns the value of the parameter having the given name, throwing an exception if the
   * parameter is missing.
   *
   * @param name The name of the parameter to return.
   * @return The value of the parameter having the given name.
   * @throws GuacamoleException If the parameter is not present in the request.
   */
  ...
  /**
   * Returns the integer value of the parameter having the given name, throwing an exception if the
   * parameter cannot be parsed.
   *
   * @param name The name of the parameter to return.
   * @return The integer value of the parameter having the given name, or null if the parameter is
   * missing.
   ...
   *
   * @return The authentication token associated with this tunnel request, or null if no
  ...
  /**
   * Returns the identifier of the AuthenticationProvider associated with the UserContext from which
   * the connection or connection group is to be retrieved when the tunnel is created. In the
   * context of the REST API and the JavaScript side of the web application, this is referred to as
   * the data source identifier.
   *
   * @return The identifier of the AuthenticationProvider associated with the UserContext from which
   * the connection or connection group is to be retrieved when the tunnel is created.
   ...
   *
   * @return The type of object for which the tunnel is being requested.
   * @throws GuacamoleException If the type was not present in the request, or if the type requested
  ...
  /**
   * Returns the identifier of the destination of the tunnel being requested. As there are multiple
   * types of destination objects available, and within multiple data sources, the associated object
   * type and data source are also necessary to determine what this identifier refers to.
   *
   ...
   *
   * @return The display width desired for the Guacamole session over the tunnel being requested, or
   * null if no width was given.
   ...
   *
   * @return The display height desired for the Guacamole session over the tunnel being requested,
   * or null if no width was given.
  ...
  /**
   * Returns the display resolution desired for the Guacamole session over the tunnel being
   * requested, in DPI.
   *
   * @return The display resolution desired for the Guacamole session over the tunnel being
   * requested, or null if no resolution was given.
   ...
   *
   * @return A list of all audio mimetypes declared as supported within the tunnel request, or null
   * if no mimetypes were specified.
   */
   ...
   *
   * @return A list of all video mimetypes declared as supported within the tunnel request, or null
   * if no mimetypes were specified.
   */
  ...
  /**
   * Returns a list of all image mimetypes declared as supported within the tunnel request.
   *
   * @return A list of all image mimetypes declared as supported within the tunnel request, or null
   * if no mimetypes were specified.
   */
  ...
  /**
   * Returns the tz database value of the timezone declared by the client within the tunnel
   * request.
   *
   * @return The tz database value of the timezone parameter as reported by the client.
   */
