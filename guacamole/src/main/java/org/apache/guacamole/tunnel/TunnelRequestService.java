...
/**
 * Utility class that takes a standard request from the Guacamole JavaScript client and produces the
 * corresponding GuacamoleTunnel. The implementation of this utility is specific to the form of
 * request used by the upstream Guacamole web application, and is not necessarily useful to
 * applications that use purely the Guacamole API.
 */
@Singleton
public class TunnelRequestService {
   ...
   *
   * @param authenticatedUser The AuthenticatedUser associated with the user for whom the tunnel is
   *                          being created.
   * @param credentials       Credentials that authenticate the user.
   * @param tunnel            The tunnel that was connected.
   * @throws GuacamoleException If thrown by a listener or if any listener vetoes the connected
   *                            tunnel.
   */
  ...
  /**
   * Notifies bound listeners that a tunnel is to be closed. Listeners are allowed to veto a request
   * to close a tunnel by throwing any GuacamoleException.
   *
   * @param authenticatedUser The AuthenticatedUser associated with the user for whom the tunnel is
   *                          being closed.
   * @param credentials       Credentials that authenticate the user.
   ...
   * @return GuacamoleClientInformation An object containing information about the client sending
   * the tunnel request.
   * @throws GuacamoleException If the parameters of the tunnel request are invalid.
   */
  ...
  /**
   * Creates a new tunnel using which is connected to the connection or connection group identifier
   * by the given ID. Client information is specified in the {@code info} parameter.
   *
   * @param context The UserContext associated with the user for whom the tunnel is being created.
  ...
  /**
   * Associates the given tunnel with the given session, returning a wrapped version of the same
   * tunnel which automatically handles closure and removal from the session.
   *
   * @param tunnel    The connected tunnel to wrap and monitor.
   * @param authToken The authentication token associated with the given session. If provided, this
   *                  token will be automatically invalidated (and the corresponding session
   *                  destroyed) if tunnel errors imply that the user is no longer authorized.
   * @param session   The Guacamole session to associate the tunnel with.
   * @param context   The UserContext associated with the user for whom the tunnel is being
   *                  created.
   * @param type      The type of object being connected to (connection or group).
   * @param id        The id of the connection or group being connected to.
   * @return A new tunnel, associated with the given session, which delegates all functionality to
   * the given tunnel while monitoring and automatically handling closure.
