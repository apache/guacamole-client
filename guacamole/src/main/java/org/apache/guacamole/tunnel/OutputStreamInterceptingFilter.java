...
/**
 * Filter which selectively intercepts "blob" and "end" instructions, automatically writing to or
 * closing the stream given with interceptStream(). The required "ack" responses to received blobs
...
public class OutputStreamInterceptingFilter
  ...
  /**
   * Whether this OutputStreamInterceptingFilter should respond to received blobs with "ack"
   * messages on behalf of the client. If false, blobs will still be handled by this filter, but
   * empty blobs will be sent to the client, forcing the client to respond on its own.
   */
  private boolean acknowledgeBlobs = true;

  /**
   * Creates a new OutputStreamInterceptingFilter which selectively intercepts "blob" and "end"
   * instructions. The required "ack" responses will automatically be sent over the given tunnel.
   *
   * @param tunnel The GuacamoleTunnel over which any required "ack" instructions should be sent.
   */
  ...
  /**
   * Injects an "ack" instruction into the outbound Guacamole protocol stream, as if sent by the
   * connected client. "ack" instructions are used to acknowledge the receipt of a stream and its
   * subsequent blobs, and are the only means of communicating success/failure status.
   *
   * @param index   The index of the stream that this "ack" instruction relates to.
   * @param message An arbitrary human-readable message to include within the "ack" instruction.
   * @param status  The status of the stream operation being acknowledged via the "ack" instruction.
   *                Error statuses will implicitly close the stream via closeStream().
   */
  ...
  /**
   * Handles a single "blob" instruction, decoding its base64 data, sending that data to the
   * associated OutputStream, and ultimately dropping the "blob" instruction such that the client
   * never receives it. If no OutputStream is associated with the stream index within the "blob"
   * instruction, the instruction is passed through untouched.
   *
   * @param instruction The "blob" instruction being handled.
   * @return The originally-provided "blob" instruction, if that instruction should be passed
   * through to the client, or null if the "blob" instruction should be dropped.
   */
  ...
  /**
   * Handles a single "end" instruction, closing the associated OutputStream. If no OutputStream is
   * associated with the stream index within the "end" instruction, this function has no effect.
   *
   * @param instruction The "end" instruction being handled.
   */
  ...
  /**
   * Handles a single "sync" instruction, updating internal tracking of client render state.
   *
   * @param instruction The "sync" instruction being handled.
   */
