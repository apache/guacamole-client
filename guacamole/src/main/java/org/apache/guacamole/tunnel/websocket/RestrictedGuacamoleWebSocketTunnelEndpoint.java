...
/**
 * Tunnel implementation which uses WebSocket as a tunnel backend, rather than HTTP, properly
 * parsing connection IDs included in the connection request.
 */
public class RestrictedGuacamoleWebSocketTunnelEndpoint extends GuacamoleWebSocketTunnelEndpoint {

  /**
   * Unique string which shall be used to store the TunnelRequest associated with a WebSocket
   * connection.
   */
  private static final String TUNNEL_REQUEST_PROPERTY = "WS_GUAC_TUNNEL_REQUEST";

  /**
   * Unique string which shall be used to store the TunnelRequestService to be used for processing
   * TunnelRequests.
   */
  ...
  protected GuacamoleTunnel createTunnel(Session session,
    ...
    TunnelRequestService tunnelRequestService = (TunnelRequestService) userProperties.get(
        TUNNEL_REQUEST_SERVICE_PROPERTY);
  ...
  /**
   * Configurator implementation which stores the requested GuacamoleTunnel within the user
   * properties. The GuacamoleTunnel will be later retrieved during the connection process.
   */
  public static class Configurator extends ServerEndpointConfig.Configurator {

    /**
     * Provider which provides instances of a service for handling tunnel requests.
     */
    private final Provider<TunnelRequestService> tunnelRequestServiceProvider;

    /**
     * Creates a new Configurator which uses the given tunnel request service provider to retrieve
     * the necessary service to handle new connections requests.
     *
     * @param tunnelRequestServiceProvider The tunnel request service provider to use for all new
     *                                     connections.
     */
