...
/**
 * A REST resource which abstracts the operations available on all Guacamole Directory
 * implementations, such as the creation of new objects, or listing of existing objects. A
 * DirectoryResource functions as the parent of any number of child DirectoryObjectResources, which
 * are created with the factory provided at the time of this object's construction.
 *
 ...
 *                       JSON) between REST clients and this DirectoryResource when representing the
 *                       InternalType.
 */
...
public abstract class DirectoryResource<InternalType extends Identifiable, ExternalType> {
  ...
  /**
   * The UserContext associated with the Directory being exposed by this DirectoryResource.
   */
  ...
  /**
   * A DirectoryObjectTranslator implementation which handles the type of objects contained within
   * the Directory exposed by this DirectoryResource.
   */
  private final DirectoryObjectTranslator<InternalType, ExternalType> translator;

  /**
   * A factory which can be used to create instances of resources representing individual objects
   * contained within the Directory exposed by this DirectoryResource.
   */
  private final DirectoryObjectResourceFactory<InternalType, ExternalType> resourceFactory;

  /**
   * Creates a new DirectoryResource which exposes the operations available for the given
   * Directory.
   *
  ...
  /**
   * Returns the ObjectPermissionSet defined within the given Permissions that represents the
   * permissions affecting objects available within this DirectoryResource.
   *
   * @param permissions The Permissions object from which the ObjectPermissionSet should be
   *                    retrieved.
   * @return The ObjectPermissionSet defined within the given Permissions object that represents the
   * permissions affecting objects available within this DirectoryResource.
  ...
  /**
   * Returns a map of all objects available within this DirectoryResource, filtering the returned
   * map by the given permission, if specified.
   *
   * @param permissions The set of permissions to filter with. A user must have one or more of these
   *                    permissions for the affected objects to appear in the result. If null, no
   *                    filtering will be performed.
   * @return A map of all visible objects. If a permission was specified, this map will contain only
   * those objects for which the current user has that permission.
  ...
  /**
   * Applies the given object patches, updating the underlying directory accordingly. This operation
   * currently only supports deletion of objects through the "remove" patch operation. The path of
   * each patch operation is of the form "/ID" where ID is the identifier of the object being
   * modified.
   *
   * @param patches The patches to apply for this request.
   * @throws GuacamoleException If an error occurs while deleting the objects.
   */
  @PATCH
  ...
  /**
   * Creates a new object within the underlying Directory, returning the object that was created.
   * The identifier of the created object will be populated, if applicable.
   *
  ...
  /**
   * Retrieves an individual object, returning a DirectoryObjectResource implementation which
   * exposes operations available on that object.
   *
   * @param identifier The identifier of the object to retrieve.
   * @return A DirectoryObjectResource exposing operations available on the object having the given
   * identifier.
