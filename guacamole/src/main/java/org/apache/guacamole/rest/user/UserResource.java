...
/**
 * A REST resource which abstracts the operations available on an existing User.
 */
...
public class UserResource
  ...
  /**
   * The UserContext associated with the Directory which contains the User exposed by this
   * resource.
   */
  private final UserContext userContext;

  /**
   * The Directory which contains the User object represented by this UserResource.
   */
  ...
  /**
   * Creates a new UserResource which exposes the operations and subresources available for the
   * given User.
   *
   * @param userContext The UserContext associated with the given Directory.
   * @param directory   The Directory which contains the given User.
   * @param user        The User that this UserResource should represent.
   * @param translator  A DirectoryObjectTranslator implementation which handles Users.
   */
  @AssistedInject
   ...
   *
   * @return A list of activity records, describing the start and end times of this user's sessions.
   * @throws GuacamoleException If an error occurs while retrieving the user history.
   */
  ...
  public UserHistoryResource getUserHistory()
    ...
    try {
      ...
      logger.debug(
          "Call to getUserHistory() is unsupported, falling back to deprecated method getHistory().",
          e);
    }
    ...
    try {
      ...
      logger.debug("Call to getHistory() is unsupported, no user history records will be returned.",
          e);
    }
   ...
   *
   * @param userPasswordUpdate The object containing the old password for the user, as well as the
   *                           new password to set for that user.
   * @param request            The HttpServletRequest associated with the password update attempt.
   * @throws GuacamoleException If an error occurs while updating the user's password.
   */
  @PUT
  @Path("password")
  ...
  /**
   * Returns a resource which abstracts operations available on the overall permissions granted
   * directly to the User represented by this UserResource.
   *
   * @return A resource which representing the permissions granted to the User represented by this
   * UserResource.
   */
  @Path("permissions")
  ...
  /**
   * Returns a read-only view of the permissions effectively granted to this user, including
   * permissions which may be inherited or implied.
   *
   * @return A read-only view of the permissions effectively granted to this user.
   * @throws GuacamoleException If the effective permissions for this user cannot be retrieved.
   */
  @GET
  ...
  /**
   * Returns a resource which abstracts operations available on the set of user groups of which the
   * User represented by this UserResource is a member.
   *
   * @return A resource which represents the set of user groups of which the User represented by
   * this UserResource is a member.
   * @throws GuacamoleException If the group membership for this user cannot be retrieved.
   */
  @Path("userGroups")
