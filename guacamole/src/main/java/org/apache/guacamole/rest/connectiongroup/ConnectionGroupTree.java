...
/**
 * Provides access to the entire tree of connection groups, their connections, and any associated
 * sharing profiles.
 */
public class ConnectionGroupTree {
  ...
  /**
   * The directory of all connection groups visible to the user obtaining this tree.
   */
  private final Directory<ConnectionGroup> connectionGroupDirectory;

  /**
   * The directory of all sharing profiles visible to the user obtaining this tree.
   */
  private final Directory<SharingProfile> sharingProfileDirectory;

  /**
   * All connection groups that have been retrieved, stored by their identifiers.
   */
   ...
   *
   * @param userContext The context of the user obtaining the connection group tree.
   * @param root        The connection group to use as the root of this connection group tree.
   * @param permissions If specified and non-empty, limit the contents of the tree to only those
   *                    connections for which the current user has any of the given permissions.
   *                    Otherwise, all visible connections are returned. Connection groups are
   *                    unaffected by this parameter.
   * @throws GuacamoleException If an error occurs while retrieving the tree of connection groups
  ...
  /**
   * Adds each of the provided connections to the current tree as children of their respective
   * parents. The parent connection groups must already be added.
   *
   * @param connections The connections to add to the tree.
   * @throws GuacamoleException If an error occurs while adding the connection to the tree.
   */
  private void addConnections(Collection<Connection> connections)
    ...
    for (Connection connection : connections) {
      ...
      // Warn of internal consistency issues
        ...
        logger.debug(
            "Connection \"{}\" cannot be added to the tree: parent \"{}\" does not actually exist.",
  ...
  /**
   * Adds each of the provided connection groups to the current tree as children of their respective
   * parents. The parent connection groups must already be added.
   *
   * @param connectionGroups The connection groups to add to the tree.
   */
  private void addConnectionGroups(Collection<ConnectionGroup> connectionGroups) {
    ...
    for (ConnectionGroup connectionGroup : connectionGroups) {
      ...
      // Warn of internal consistency issues
        ...
        logger.debug(
            "Connection group \"{}\" cannot be added to the tree: parent \"{}\" does not actually exist.",
  ...
  /**
   * Adds each of the provided sharing profiles to the current tree as children of their respective
   * primary connections. The primary connections must already be added.
   *
   * @param sharingProfiles The sharing profiles to add to the tree.
   * @throws GuacamoleException If an error occurs while adding the sharing profiles to the tree.
   */
  ...
  /**
   * Adds all descendants of the given parent groups to their corresponding parents already stored
   * under root.
   *
   * @param parents     The parents whose descendants should be added to the tree.
   * @param permissions If specified and non-empty, limit added connections to only connections for
   *                    which the current user has any of the given permissions. Otherwise, all
   *                    visible connections are added. Connection groups are unaffected by this
   *                    parameter.
   * @throws GuacamoleException If an error occurs while retrieving the descendants.
   */
  private void addConnectionGroupDescendants(Collection<ConnectionGroup> parents,
    ...
    if (!childConnectionIdentifiers.isEmpty()) {
      Collection<Connection> childConnections = connectionDirectory.getAll(
    ...
    if (!childConnectionGroupIdentifiers.isEmpty()) {
      Collection<ConnectionGroup> childConnectionGroups = connectionGroupDirectory.getAll(
          childConnectionGroupIdentifiers);
  ...
  /**
   * Adds all descendant sharing profiles of the given connections to their corresponding primary
   * connections already stored under root.
   *
   * @param connections The connections whose descendant sharing profiles should be added to the
   *                    tree.
   * @param permissions If specified and non-empty, limit added sharing profiles to only those for
   *                    which the current user has any of the given permissions. Otherwise, all
   *                    visible sharing profiles are added.
   * @throws GuacamoleException If an error occurs while retrieving the descendants.
   */
  ...
  /**
   * Returns the entire connection group tree as an APIConnectionGroup. The returned
   * APIConnectionGroup is the root group and will contain all descendant connection groups and
   * connections, arranged hierarchically.
   *
   * @return The root connection group, containing the entire connection group tree and all
   * connections.
   */
