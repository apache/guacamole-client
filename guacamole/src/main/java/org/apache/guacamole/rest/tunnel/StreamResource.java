...
/**
 * A REST resource providing access to a Guacamole protocol-level stream within a tunnel.
 */
...
public class StreamResource {
   ...
   *
   * @param tunnel      The tunnel whose stream is being exposed.
   * @param streamIndex The index of the stream to expose via this StreamResource.
   * @param mediaType   The media type of the data within the stream.
   */
  ...
  /**
   * Intercepts and returns the entire contents the stream represented by this StreamResource.
   *
  ...
  /**
   * Intercepts the stream represented by this StreamResource, sending the contents of the given
   * InputStream over that stream as "blob" instructions.
   *
   * @param data An InputStream containing the data to be sent over the intercepted stream.
   * @throws GuacamoleException If the intercepted stream closes with an error.
   */
  @POST
