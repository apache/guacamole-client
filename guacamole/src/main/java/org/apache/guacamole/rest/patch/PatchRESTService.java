...
public class PatchRESTService {
  ...
  /**
   * Service for retrieving information regarding available HTML patch resources.
   */
  @Inject
  private PatchResourceService patchResourceService;

  /**
   * Reads the entire contents of the given resource as a String. The resource is assumed to be
   * encoded in UTF-8.
   *
   * @param resource The resource to read as a new String.
   * @return A new String containing the contents of the given resource.
   * @throws IOException If an I/O error prevents reading the resource.
   */
  ...
  /**
   * Returns a list of all available HTML patches, in the order they should be applied. Each patch
   * is raw HTML containing additional meta tags describing how and where the patch should be
   * applied.
   *
   * @return A list of all HTML patches defined in the system, in the order they should be applied.
   * @throws GuacamoleException If an error occurs preventing any HTML patch from being read.
   */
  @GET
