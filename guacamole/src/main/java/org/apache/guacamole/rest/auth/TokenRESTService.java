...
public class TokenRESTService {
  ...
  /**
   * Returns the credentials associated with the given request, using the provided username and
   * password.
   *
   * @param request  The request to use to derive the credentials.
   * @param username The username to associate with the credentials, or null if the username should
   *                 be derived from the request.
   * @param password The password to associate with the credentials, or null if the password should
   *                 be derived from the request.
   * @return A new Credentials object whose contents have been derived from the given request, along
  ...
  /**
   * Authenticates a user, generates an auth token, associates that auth token with the user's
   * UserContext for use by further requests. If an existing token is provided, the authentication
   * procedure will attempt to update or reuse the provided token.
   *
   ...
   * @param token           An optional existing auth token for the user who is to be
   *                        authenticated.
   * @param consumedRequest The HttpServletRequest associated with the login attempt. The parameters
   *                        of this request may not be accessible, as the request may have been
   *                        fully consumed by JAX-RS.
   * @param parameters      A MultivaluedMap containing all parameters from the given HTTP request.
   *                        All request parameters must be made available through this map, even if
   *                        those parameters are no longer accessible within the now-fully-consumed
   *                        HTTP request.
   * @return An authentication response object containing the possible-new auth token, as well as
   * other related data.
   * @throws GuacamoleException If an error prevents successful authentication.
   */
  @POST
   ...
   * @throws GuacamoleException If the specified token does not exist.
   */
  @DELETE
  @Path("/{token}")
