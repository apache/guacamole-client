...
/**
 * Authenticates users against a static list of username/password pairs. Each username/password may
 * be associated with multiple configurations. This list is stored in an XML file which is reread if
 * modified.
 */
public class FileAuthenticationProvider extends SimpleAuthenticationProvider {
  ...
  /**
   * The time the user mapping file was last modified. If the file has never been read, and thus no
   * modification time exists, this will be Long.MIN_VALUE.
   */
   ...
   * GUACAMOLE_HOME/user-mapping.xml. If the XML file has been modified or has not yet been read,
   * this function may reread the file.
   *
   * @return A UserMapping containing all authorization data within the user mapping XML file, or
   * null if the file cannot be found/parsed.
   */
  private UserMapping getUserMapping() {
    ...
    if (!userMappingFile.exists()) {
      logger.debug("User mapping file \"{}\" does not exist and will not be read.",
          userMappingFile);
      return null;
    }
    ...
    if (lastModified < userMappingFile.lastModified()) {
      ...
      try {
        ...
        logger.error("Unable to create XML parser for reading \"{}\": {}", USER_MAPPING_FILENAME,
            e.getMessage());
