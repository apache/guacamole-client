...
/**
 * Contains Guacamole-specific user information which is tied to the current session, such as the
 * UserContext and current clipboard state.
 */
public class GuacamoleSession {
  ...
  /**
   * All UserContexts associated with this session. Each AuthenticationProvider may provide its own
   * UserContext.
   */
   ...
   *
   * @param environment       The environment of the Guacamole server associated with this new
   *                          session.
   * @param authenticatedUser The authenticated user to associate this session with.
   * @param userContexts      The List of UserContexts to associate with this session.
   * @throws GuacamoleException If an error prevents the session from being created.
   */
  ...
  /**
   * Replaces the authenticated user associated with this session with the given authenticated
   * user.
   *
   * @param authenticatedUser The authenticated user to associated with this session.
   */
  ...
  /**
   * Returns a list of all UserContexts associated with this session. Each AuthenticationProvider
   * currently loaded by Guacamole may provide its own UserContext for any
   * successfully-authenticated user.
   *
   * @return An unmodifiable list of all UserContexts associated with this session.
   */
  ...
  /**
   * Returns the UserContext associated with this session that originated from the
   * AuthenticationProvider with the given identifier. If no such UserContext exists, an exception
   * is thrown.
   *
   * @param authProviderIdentifier The unique identifier of the AuthenticationProvider that created
   *                               the UserContext being retrieved.
   * @return The UserContext that was created by the AuthenticationProvider having the given
   * identifier.
   * @throws GuacamoleException If no such UserContext exists.
   */
   ...
   *
   * @return true if this session has any associated active tunnels, false otherwise.
   */
  ...
  /**
   * Returns a map of all active tunnels associated with this session, where each key is the String
   * representation of the tunnel's UUID. Changes to this map immediately affect the set of tunnels
   * associated with this session. A tunnel need not be present here to be used by the user
   * associated with this session, but tunnels not in this set will not be taken into account when
   * determining whether a session is in use.
   *
  ...
  /**
   * Associates the given tunnel with this session, such that it is taken into account when
   * determining session activity.
   *
  ...
  /**
   * Returns the time this session was last accessed, as the number of milliseconds since midnight
   * January 1, 1970 GMT. Session access must be explicitly marked through calls to the access()
   * function.
   *
