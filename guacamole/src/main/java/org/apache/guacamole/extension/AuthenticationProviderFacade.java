...
/**
 * Provides a safe wrapper around an AuthenticationProvider subclass, such that authentication
 * attempts can cleanly fail, and errors can be properly logged, even if the AuthenticationProvider
 * cannot be instantiated.
 */
public class AuthenticationProviderFacade implements AuthenticationProvider {

  /**
   * The underlying authentication provider, or null if the authentication provider could not be
   * instantiated.
   */
  private final AuthenticationProvider authProvider;
  /**
   * The set of identifiers of all authentication providers whose internal failures should be
   * tolerated during the authentication process. If the identifier of this authentication provider
   * is within this set, errors during authentication will result in the authentication provider
   * being ignored for that authentication attempt. By default, errors during authentication halt
   * the authentication process entirely.
   */
  private final Set<String> tolerateFailures;
  /**
   * The identifier to provide for the underlying authentication provider if the authentication
  ...
  /**
   * Creates a new AuthenticationProviderFacade which delegates all function calls to an instance of
   * the given AuthenticationProvider subclass. If an instance of the given class cannot be created,
   * creation of this facade will still succeed, but its use will result in errors being logged, and
   * all authentication attempts will fail.
   *
   * @param authProviderClass The AuthenticationProvider subclass to instantiate.
  ...
  public String getIdentifier() {
    ...
    if (authProvider == null) {
      logger.warn(
          "The authentication system could not be loaded. Please check for errors earlier in the logs.");
  ...
  /**
   * Returns whether this authentication provider should tolerate internal failures during the
   * authentication process, allowing other authentication providers to continue operating as if
   * this authentication provider simply is not present.
   *
   * @return true if this authentication provider should tolerate internal failures during the
   * authentication process, false otherwise.
   */
  ...
  /**
   * Logs a warning that this authentication provider is being skipped due to an internal error. If
   * debug-level logging is enabled, the full details of the internal error are also logged.
   *
   * @param e The internal error that occurred which has resulted in this authentication provider
   *          being skipped.
   */
  ...
  /**
   * Logs a warning that the authentication process will be entirely aborted due to an internal
   * error, advising the administrator to set the "skip-if-unavailable" property if error
   * encountered is expected and should be tolerated.
   */
