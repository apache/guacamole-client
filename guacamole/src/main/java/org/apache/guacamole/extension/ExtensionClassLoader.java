...
/**
 * ClassLoader implementation which prioritizes the classes defined within a given extension .jar
 * file. Unlike the standard URLClassLoader, classes within the parent ClassLoader are only used if
 * they are not defined within the given .jar. If classes are defined in both the parent and the
 * extension .jar, the versions defined within the extension .jar are used.
 */
public class ExtensionClassLoader extends URLClassLoader {
  ...
  /**
   * The prefix that should be given to the temporary directory containing all library .jar files
   * that were bundled with the extension.
   */
  private static final String EXTENSION_TEMP_DIR_PREFIX = "guac-extension-lib-";

  /**
   * The prefix that should be given to any files created for temporary storage of a library .jar
   * file that was bundled with the extension.
   */
  private static final String EXTENSION_TEMP_LIB_PREFIX = "bundled-";

  /**
   * The ClassLoader to use if class resolution through the extension .jar fails.
   */
  private final ClassLoader parent;

  /**
   * Creates a new ExtensionClassLoader configured to load classes from the given extension .jar. If
   * a necessary class cannot be found within the .jar, the given parent ClassLoader is used.
   * Calling this function multiple times will not affect previously-returned instances of
   * ExtensionClassLoader.
   *
   * @param extension      The extension .jar file from which classes should be loaded.
   ...
   * @param parent         The ClassLoader to use if class resolution through the extension .jar
   *                       fails.
   * @throws GuacamoleException If the given file is not actually a file, or the contents of the
   *                            file cannot be read.
   */
  ...
  /**
   * Returns an instance of ExtensionClassLoader configured to load classes from the given extension
   * .jar. If a necessary class cannot be found within the .jar, the given parent ClassLoader is
   * used. Calling this function multiple times will not affect previously-returned instances of
   * ExtensionClassLoader.
   *
   * @param extension      The extension .jar file from which classes should be loaded.
   * @param temporaryFiles A modifiable List that should be populated with all temporary files
   ...
   * @param parent         The ClassLoader to use if class resolution through the extension .jar
   *                       fails.
   * @return A ExtensionClassLoader instance which loads classes from the given extension .jar file.
   * @throws GuacamoleException If the given file is not actually a file, or the contents of the
   *                            file cannot be read.
   */
  ...
  /**
   * Returns the URL that refers to the given file. If the given file refers to a directory, an
   * exception is thrown.
   *
   * @param file The file to determine the URL of.
   * @return A URL that refers to the given file.
   * @throws GuacamoleException If the given file refers to a directory.
   */
  ...
  /**
   * Copies all bytes of data from a file within a .jar to a destination file.
   *
   * @param jar    The JarFile containing the file to be copied.
   * @param source The JarEntry representing the file to be copied within the given JarFile.
   * @param dest   The destination file that the data should be copied to.
   * @throws IOException If an error occurs reading from the source .jar or writing to the
   *                     destination file.
   */
  ...
  /**
   * Returns the URLs for the .jar files relevant to the given extension .jar file. Unless the
   * extension bundles additional Java libraries, only the URL of the extension .jar will be
   * returned. If additional Java libraries are bundled within the extension, URLs for those
   * libraries will be included, as well. Temporary directories and/or files will be created as
   * necessary to house bundled libraries. Only .jar files located directly within the root of the
   * main extension .jar are considered.
   *
   * @param extension      The extension .jar file to generate URLs for.
   * @param temporaryFiles A modifiable List that should be populated with all temporary files
   *                       created for the given extension. These files should be deleted on
   *                       application shutdown in reverse order.
   * @return An array of all URLs relevant to the given extension .jar.
   * @throws GuacamoleException If the given file is not actually a file, the contents of the file
  ...
  private static URL[] getExtensionURLs(File extension,
    ...
    try (JarFile extensionJar = new JarFile(extension)) {
      ...
      while (entries.hasMoreElements()) {
        ...
        File tempLibrary;
        try {
          tempLibrary = Files.createTempFile(extensionTempLibDir, EXTENSION_TEMP_LIB_PREFIX, ".jar")
              .toFile();
