...
Guacamole.Mouse = function Mouse(element) {
  ...
  element.addEventListener("mousemove", function (e) {
    ...
    guac_mouse.move(
        Guacamole.Position.fromClientPosition(element, e.clientX, e.clientY),
        e);
...
Guacamole.Mouse.State = function State(template) {
   ...
   */
  var legacyConstructor = function legacyConstructor(x, y, left, middle, right,
      up, down) {
    return {
      x: x,
      y: y,
...
Guacamole.Mouse.Event.Target = function MouseEventTarget() {
  ...
  this.press = function press(button, events) {
    if (!this.currentState[button]) {
      ...
      this.dispatch(
          new Guacamole.Mouse.Event('mousedown', this.currentState, events));
    }
  };
  ...
  this.move = function move(position, events) {
    ...
    if (this.currentState.x !== position.x || this.currentState.y
        !== position.y) {
      this.currentState.x = position.x;
      this.currentState.y = position.y;
      this.dispatch(
          new Guacamole.Mouse.Event('mousemove', this.currentState, events));
    }
    ...
    if (!finger_moved(e)) {
      ...
      if (!guac_touchscreen.currentState.left) {
        ...
        guac_touchscreen.move(
            Guacamole.Position.fromClientPosition(element, touch.clientX,
                touch.clientY));
