...
Guacamole.Display = function () {
  ...
  var asyncFlush = function asyncFlush() {
    ...
    var continueFlush = function continueFlush() {
      ...
      if (frames[0].isReady()) {
        ...
        frame.flush();
        notifyFlushed(frame.localTimestamp, frame.remoteTimestamp,
            frame.logicalFrames);
      }
   ...
   */
  var notifyFlushed = function notifyFlushed(localTimestamp, remoteTimestamp,
      logicalFrames) {
    ...
    for (var first = 0; first < statistics.length; first++) {
      if (current - statistics[first].timestamp
    ...
    // bound)
    var statDuration = (statistics[statistics.length - 1].timestamp
        - statistics[0].timestamp) / 1000;

    // Determine the amount of time that elapsed remotely (within the
    // remote desktop)
    var remoteDuration = (statistics[statistics.length - 1].remoteTimestamp
        - statistics[0].remoteTimestamp) / 1000;
    ...
    var stats = new Guacamole.Display.Statistics({
      ...
      desktopFps: (remoteDuration && remoteFrames) ? remoteFrames
          / remoteDuration : null,
   ...
   *     space to copy data from.
   */
  this.setCursor = function (hotspotX, hotspotY, layer, srcx, srcy, srcw,
      srch) {
   ...
   *     destination.
   */
  this.transfer = function (srcLayer, srcx, srcy, srcw, srch, dstLayer, x, y,
      transferFunction) {
    scheduleTask(function __display_transfer() {
      dstLayer.transfer(srcLayer, srcx, srcy, srcw, srch, x, y,
          transferFunction);
    });
  };
