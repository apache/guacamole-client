...
Guacamole.SessionRecording = function SessionRecording(source) {
   ...
   *     the given blob.
   */
  var parseBlob = function parseBlob(blob, instructionCallback,
      completionCallback) {
  ...
  var loadInstruction = function loadInstruction(opcode, args) {
    ...
    if (opcode === 'sync') {
      ...
      var frame = new Guacamole.SessionRecording._Frame(timestamp, frameStart,
          frameEnd);
      frames.push(frame);
      frameStart = frameEnd;
      ...
      if (frames.length === 1 || (frameEnd - frames[lastKeyframe].start
  ...
  var replayFrame = function replayFrame(index, callback) {
    ...
    parseBlob(recordingBlob.slice(frame.start, frame.end),
  ...
  var seekToFrame = function seekToFrame(index, callback, nextRealTimestamp) {
    ...
    var continueAfterRequiredDelay = function continueAfterRequiredDelay() {
      var delay = nextRealTimestamp ? Math.max(
          nextRealTimestamp - new Date().getTime(), 0) : 0;
      if (delay) {
