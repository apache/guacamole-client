...
Guacamole.Client = function (tunnel) {
  ...
  this.createOutputStream = function createOutputStream() {
    ...
    // Return new stream
    var stream = output_streams[index] = new Guacamole.OutputStream(guac_client,
        index);
    return stream;
   ...
   */
  this.createArgumentValueStream = function createArgumentValueStream(mimetype,
      name) {
   ...
   *     of the given object.
   */
  this.createObjectOutputStream = function createObjectOutputStream(index,
      mimetype, name) {
   ...
   */
  this.requestObjectInputStream = function requestObjectInputStream(index,
      name) {
  ...
  var layerPropertyHandlers = {
    ...
    "multi-touch": function layerSupportsMultiTouch(layer, value) {
      ...
      if (guac_client.onmultitouch && layer
          instanceof Guacamole.Display.VisibleLayer) {
  ...
  var instructionHandlers = {
    ...
    "argv": function (parameters) {
      ...
      // Otherwise, unsupported
        ...
        guac_client.sendAck(stream_index,
            "Receiving argument values unsupported", 0x0100);
      }
    ...
    "filesystem": function handleFilesystem(parameters) {
      ...
      if (guac_client.onfilesystem) {
        var object = objects[objectIndex] = new Guacamole.Object(guac_client,
            objectIndex);
    ...
    "name": function (parameters) {
      if (guac_client.onname) {
        guac_client.onname(parameters[0]);
      }
    },
    ...
    "required": function required(parameters) {
      if (guac_client.onrequired) {
        guac_client.onrequired(parameters);
      }
    },
    ...
    "video": function (parameters) {
      ...
      // If unsuccessful, try to use a default implementation
      if (!videoPlayer) {
        videoPlayer = Guacamole.VideoPlayer.getInstance(stream, layer,
            mimetype);
      }
